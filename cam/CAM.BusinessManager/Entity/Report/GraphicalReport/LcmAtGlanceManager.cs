...
{
    public class LcmAtGlanceManager : BaseManager
    {
        ...
        public LcmAtGlanceManager(IEnumerable<IRepositoryWrapper> wrappers,
        ...
        public async Task<ResultDto> FindWithConditionForOverAllOpco()
        {
            ...
            var overAllOpcoPercentage = await  GetOverAllOpcoAndAssetWisePercentage(lcmGlanceEntity, overAllCount, overAllNodeCount);
       ...
       m =>
       {
        ...
        public async Task<LcmAtGlanceGridDto>  GetOverAllOpcoAndAssetWisePercentage(  IEnumerable<LcmAtGlanceGridDto> lcmGlanceEntity, int overAllCount, int overAllNodeCount)
        {
            ...
            short? GetNodeCount(string colorCode) => colorCounts.TryGetValue(colorCode, out var data) ? data.NodeCount : 0;
            ...
            return new LcmAtGlanceGridDto
            {
                ...
                // Node counts
                greenNodeCount = GetNodeCount(ConstantValueFilter.Green),
                amberNodeCount = GetNodeCount(ConstantValueFilter.Amber),
                redNodeCount = GetNodeCount(ConstantValueFilter.Red),
                NodesCount = (long)lcmGlanceEntity.Sum(e => e.NodesCount),

                // Node percentages
                greenNodePercentage = CalculateNodePercentage(GetNodeCount(ConstantValueFilter.Green), overAllNodeCount),
                amberNodePercentage = CalculateNodePercentage(GetNodeCount(ConstantValueFilter.Amber), overAllNodeCount),
                redNodePercentage = CalculateNodePercentage(GetNodeCount(ConstantValueFilter.Red), overAllNodeCount)
            };
        }
