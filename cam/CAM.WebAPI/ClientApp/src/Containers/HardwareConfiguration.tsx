...
import {
  HardwareConfigurationDtoCreate,
  HardwareConfigurationDtoGrid,
  HardwareConfigurationDtoUpdate,
  HardwareConfigurationQueryObjectGrid,
} from "../Model/HardwareConfiguration";
...
import { GetHardwareConfigurationReport } from "../Redux/Action/HardwareConfiguration/HardwareConfigurationDownloadAction";
import { GetNetworkElementAsIsEditResource } from "../Redux/Action/NetworkElementAsIs/NetworkElementAsIsEditAction";
import { GetHardwareConfigurationGrid } from "../Redux/Action/HardwareConfiguration/HardwareConfigurationGridAction";
import { RootState } from "../Redux/Store/rootStore";
import HardwareConfigurationGrid from "../screen/HardwareConfiguration/HardwareConfigurationGrid";
...
export let paginationQuery: HardwareConfigurationQueryObjectGrid = {
...
const HardwareConfiguration: React.FC = (props) => {
  //STATE CONFIRM
  ...
  //DTO
  const [data, setData] = useState<HardwareConfigurationDtoGrid[] | undefined>(
    []
  );
  const Grid = (state: RootState) =>
    state.hardwareConfigurationGridReducer.HardwareConfigurationGridResult;
  ...
  const { query, setQuery, next, back } = useResourceTableCrud(
    paginationQuery,
    isPermesso ? GetHardwareConfigurationGrid : undefined
  );
  ...
  const refresh = () => {
    ...
    closeModal();
    GetHardwareConfigurationGrid(query).then(() =>
  ...
  useEffect(() => {
    ...
    if (location.state != null && location.state !== undefined) {
      ...
      if (localState?.id != null) {
        ...
        let copy = { ...query } as HardwareConfigurationQueryObjectGrid;
        copy.hardwareconfigurationid = [];
        copy.elementName = [localState?.hostname ?? ""];
        copy.hardwareconfigurationid?.push(localState?.id);
        copy.principalId = localState?.id;
        setQuery(copy);
        GetHardwareConfigurationGrid(copy).then((x) =>
          setLoader("REMOVE", "GetHardwareConfigurationGrid")
        );
      }
      if (
        localState?.hostname !== "" &&
        localState?.hostname !== undefined &&
        localState?.hostname !== null
      ) {
        setLocalState(localState);
        setRedirect(true);
        setFilterRedirect(true);
      ...
      // GetHardwareConfigurationGrid(paginationQuery).then((x) =>
      //   setLoader("REMOVE", "GetHardwareConfigurationGrid")
      // );
    }
    setLoader("REMOVE", "GetHardwareConfigurationGrid");
  ...
  const closeModalSetup = (changed: boolean) => {
    GetHardwareConfigurationGrid(query).then((x) =>
      setIsVisibleModalSetup(false)
    );
  };

  const InvocheDownload = async () => {
    let result = await GetHardwareConfigurationReport(query);
    if (result !== undefined) {
        ...
        <div className="d-flex flex-row align-items-center">
          {redirect === true ? (
            <>
              ...
              location?.state?.hostname !== undefined &&
              location?.state?.hostname !== null ? (
                <Link
                  className="d-flex justify-content-center align-items-center mr-3 mb-2"
                  to={{
                    pathname: `/generatelcmdbR10`,
                    search: "",
                  }}
                  state={{
                    id: undefined,
          ...
          <h3 className="voda-bold">Hardware Configuration</h3>
        ...
        <div className="d-flex">
          {/* {!readonly && (
            ...
            >
              ...
              <span className="fz-14">New HardwareConfiguration</span>
            </button>
          )} */}
