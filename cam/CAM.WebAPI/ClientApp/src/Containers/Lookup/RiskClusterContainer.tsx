...
import {
  GetRiskClusterCreateResource,
  GetRiskClusterEditResource,
  GetRiskClusterGrid,
  GetRiskClusterGridALL,
  deleteRiskCluster,
...
import RiskClusterForm from "../../screen/Lookup/RiskCluster/RiskClusterForm";
import RiskClusterGrid from "../../screen/Lookup/RiskCluster/RiskClusterGrid";
...
const RiskCluster: React.FC<Props> = (props) => {
  const { isPermesso } = useAuth();
  //DTO
  const [data, setData] = useState<TipologicaGridDto[] | undefined>([]);
  const Grid = (state: RootState) =>
    state.riskClusterGridReducer.LookUpGridResult;
  const GridDto = useSelector(Grid);
  const GridAll = (state: RootState) =>
    state.riskClusterGridReducer.LookUpGridResultAll;
  ...
  const refresh = () => {
    closeModal();
    GetRiskClusterGrid(query);
  };

  //PAGINAZIONE E RISULTATI FILTRAGGIO
  const { query, setQuery, next, back } = useResourceTableCrud(
    paginationQueryTipologiche,
    isPermesso ? GetRiskClusterGrid : undefined
  );
  ...
  // useEffect(() => {
  //   GetRiskClusterGrid(paginationQueryTipologiche);
  //   GetRiskClusterGridALL();
  // }, []);
    ...
    >
      ...
      >
        <DialogTitle className="d-flex justify-content-center">
          <div className="col-12">
            <h4 className="mb-0">
              {edit ? " Edit Risk Cluster" : "Add Risk Cluster"}
            </h4>
      ...
      <div className="headerPage row mx-0 justify-content-between">
        <div className="d-flex flex-row align-items-center">
          <h3 className="voda-bold text-dark">Risk Cluster</h3>
        </div>
        <div className="">
          <button
