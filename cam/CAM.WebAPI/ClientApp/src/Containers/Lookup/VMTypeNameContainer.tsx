...
import VMTypeNameForm from "../../screen/Lookup/VMTypeName/VMTypeNameForm";
import VMTypeNameGrid from "../../screen/Lookup/VMTypeName/VMTypeNameGrid";
import { GetVMTypeNameCreateResource } from "../../Redux/Action/LookUp/VMTypeName/VMTypeNameCreateAction";
import {
  DeleteDeepVMTypeName,
  DeleteVMTypeName,
} from "../../Redux/Action/LookUp/VMTypeName/VMTypeNameDeleteAction";
import {
  GetVMTypeNameGrid,
  GetVMTypeNameGridALL,
...
import { GetVMTypeNameEditResource } from "../../Redux/Action/LookUp/VMTypeName/VMTypeNameEditAction";

export let paginationQueryTipologiche: VMTypeNameQueryObjectGrid = {
  vmTypeNameId: [],
  vmTypeDescription: [],
  vnfName: [],
  vnfNameId: [],
...
const VMTypeName: React.FC<Props> = (props) => {
  const { isPermesso } = useAuth();
  //DTO
  const [data, setData] = useState<VMTypeNameDtoGrid[] | undefined>([]);
  const [changed, setChanged] = useState<boolean>();
  const Grid = (state: RootState) =>
    state.vMTypeNameGridReducer.LookUpGridResult;
  const GridDto = useSelector(Grid);
  const GridAll = (state: RootState) =>
    state.vMTypeNameGridReducer.LookUpGridResultAll;
  ...
  const refresh = () => {
    closeModal();
    GetVMTypeNameGrid(query);
  };

  //PAGINAZIONE E RISULTATI FILTRAGGIO
  const { query, setQuery, next, back } = useResourceTableCrud(
    paginationQueryTipologiche,
    isPermesso ? GetVMTypeNameGrid : undefined
  );
  ...
  const {
    ...
    localStateHistory,
    setLocalState,
  } = useOperationTableCrud<VMTypeNameDtoGrid, VMTypeNameDtoGrid>(
    ...
    <div>
      ...
      >
        <DialogTitle className="d-flex justify-content-center">
          <div className="col-12">
            <h4 className="mb-0">
              {edit ? " Edit VNF VMType Name" : "Add VNF VMType Name"}
            </h4>
          </div>
        </DialogTitle>
        <IconButton
      ...
      <div className="headerPage row mx-0 justify-content-between">
        <div className="d-flex flex-row align-items-center">
          <h3 className="voda-bold text-dark">VNF VMType Name</h3>
        </div>
        <div className="">
          <button
