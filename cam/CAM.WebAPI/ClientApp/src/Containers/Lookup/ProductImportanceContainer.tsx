...
import { GetProductImportanceCreateResource } from "../../Redux/Action/LookUp/ProductImportance/ProductImportanceCreateAction";
...
import ProductImportanceGrid from "../../screen/Lookup/ProductImportance/ProductImportanceGrid";
import ProductImportanceForm from "../../screen/Lookup/ProductImportance/ProductImportanceForm";
...
const ProductImportanceContainer: React.FC<Props> = (props) => {
  const { isPermesso } = useAuth();
  //DTO
  const [data, setData] = useState<TipologicaGridDto[] | undefined>([]);
  const [changed, setChanged] = useState<boolean>();
  const Grid = (state: RootState) =>
    state.productImportanceGridReducer.LookUpGridResult;
  ...
  const refresh = () => {
    closeModal();
    GetProductImportanceGrid(query);
  };

  //PAGINAZIONE E RISULTATI FILTRAGGIO
  const { query, setQuery, next, back } = useResourceTableCrud(
    paginationQueryTipologiche,
    isPermesso ? GetProductImportanceGrid : undefined
  );
  ...
  const {
    ...
    GetProductImportanceCreateResource,
    GetProductImportanceEditResource,
    DeleteDeepProductImportance,
    refresh
  );
  ...
  // useEffect(() => {
  //   GetProductImportanceGrid(paginationQueryTipologiche);
  // }, []);
  ...
  const onDelete = async (id: number) => {
    const result = await GetRelatedRecordsProductImportance(id);
    if (result.data != null) {
    ...
    >
      ...
      >
        <DialogTitle className="d-flex justify-content-center">
          <div className="col-12">
            <h4 className="mb-0">
              {edit ? "Edit Product Importance" : "New Product Importance"}
            </h4>
      ...
      <div className="headerPage row mx-0 justify-content-between">
        <div className="d-flex flex-row align-items-center">
          <h3 className="voda-bold text-dark">Product Importance</h3>
        </div>
        <div className="">
          <button
...
export default ProductImportanceContainer;
