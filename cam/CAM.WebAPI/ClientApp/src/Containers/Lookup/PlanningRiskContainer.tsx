...
import PlanningRiskForm from "../../screen/Lookup/PlanningRisk/PlanningRiskForm";
import PlanningRiskGrid from "../../screen/Lookup/PlanningRisk/PlanningRiskGrid";
import { GetPlanningRiskCreateResource } from "../../Redux/Action/LookUp/PlanningRisk/PlanningRiskCreateAction";
import {
  DeleteDeepPlanningRisk,
  GetRelatedRecordsPlanningRisk,
} from "../../Redux/Action/LookUp/PlanningRisk/PlanningRiskDeleteAction";
import { GetPlanningRiskEditResource } from "../../Redux/Action/LookUp/PlanningRisk/PlanningRiskEditAction";
import {
  GetPlanningRiskGrid,
  GetPlanningRiskGridALL,
} from "../../Redux/Action/LookUp/PlanningRisk/PlanningRiskGridAction";
import {
...
const PlanningRisk: React.FC<Props> = (props) => {
  const { isPermesso } = useAuth();
  //DTO
  const [data, setData] = useState<TipologicaGridDto[] | undefined>([]);
  const Grid = (state: RootState) =>
    state.planningRiskGridReducer.LookUpGridResult;
  const GridDto = useSelector(Grid);
  const GridAll = (state: RootState) =>
    state.planningRiskGridReducer.LookUpGridResultAll;
  ...
  const refresh = () => {
    closeModal();
    GetPlanningRiskGrid(query);
  };

  //PAGINAZIONE E RISULTATI FILTRAGGIO
  const { query, setQuery, next, back } = useResourceTableCrud(
    paginationQueryTipologiche,
    isPermesso ? GetPlanningRiskGrid : undefined
  );
  ...
  const {
    ...
    GetPlanningRiskCreateResource,
    GetPlanningRiskEditResource,
    DeleteDeepPlanningRisk,
    refresh
  );
  ...
  useEffect(() => {
    GetPlanningRiskGrid(paginationQueryTipologiche);
    GetPlanningRiskGridALL();
  }, []);
  ...
  const onDelete = async (id: number) => {
    const result = await GetRelatedRecordsPlanningRisk(id);
    if (result.data != null) {
    ...
    >
      ...
      >
        <Modal.Header closeButton>
          <div className="col-12">
            <h4 className="mb-0">
              {edit ? " Edit Planning Risk" : "Create Planning Risk"}
            </h4>
      ...
      <div className="headerPage row mx-0 justify-content-between">
        <div className="d-flex flex-row align-items-center">
          <h3 className="voda-bold text-dark">Planning Risk</h3>
        </div>
        <div className="">
          <button
