...
import HardwareTypeForm from "../../screen/Lookup/HardwareType/HardwareTypeForm";
import HardwareTypeGrid from "../../screen/Lookup/HardwareType/HardwareTypeGrid";
import { GetHardwareTypeCreateResource } from "../../Redux/Action/LookUp/HardwareType/HardwareTypeCreateAction";
import {
  DeleteDeepHardwareType,
  GetRelatedRecordsHardwareType,
} from "../../Redux/Action/LookUp/HardwareType/HardwareTypeDeleteAction";
import { GetHardwareTypeEditResource } from "../../Redux/Action/LookUp/HardwareType/HardwareTypeEditAction";
import {
  GetHardwareTypeGrid,
  GetHardwareTypeGridALL,
...
const HardwareType: React.FC<Props> = (props) => {
  const { isPermesso } = useAuth();
  //DTO
  const [data, setData] = useState<TipologicaGridDto[] | undefined>([]);
  const [changed, setChanged] = useState<boolean>();
  const Grid = (state: RootState) =>
    state.hardwareTypeGridReducer.LookUpGridResult;
  const GridAll = (state: RootState) =>
    state.hardwareTypeGridReducer.LookUpGridResultAll;
  ...
  const refresh = () => {
    closeModal();
    GetHardwareTypeGrid(query);
  };

  //PAGINAZIONE E RISULTATI FILTRAGGIO
  const { query, setQuery, next, back } = useResourceTableCrud(
    paginationQueryTipologiche,
    isPermesso ? GetHardwareTypeGrid : undefined
  );
  ...
  useEffect(() => {
    GetHardwareTypeGrid(paginationQueryTipologiche);
    GetHardwareTypeGridALL();
  }, []);
  ...
  const onDelete = async (id: number) => {
    const result = await GetRelatedRecordsHardwareType(id);
    if (result.data != null) {
    ...
    >
      ...
      <div className="headerPage row mx-0 justify-content-between">
        <div className="d-flex flex-row align-items-center">
          <h3 className="voda-bold">Hardware Type</h3>
        </div>
        <div className="">
          <button
