...
import { GetPracticeCreateResource } from "../../Redux/Action/LookUp/Practice/PracticeCreateAction";
import {
  DeleteDeepPractice,
  deletePractice,
} from "../../Redux/Action/LookUp/Practice/PracticeDeleteAction";
import { GetPracticeEditResource } from "../../Redux/Action/LookUp/Practice/PracticeEditAction";
import {
  GetPracticeGrid,
  GetPracticeGridALL,
} from "../../Redux/Action/LookUp/Practice/PracticeGridAction";
import {
...
import PracticeForm from "../../screen/Lookup/Practice/PracticeForm";
import PracticeGrid from "../../screen/Lookup/Practice/PracticeGrid";
...
const Practice: React.FC<Props> = (props) => {
  const { isPermesso } = useAuth();
  //DTO
  const [data, setData] = useState<TipologicaGridDto[] | undefined>([]);
  const Grid = (state: RootState) => state.practiceGridReducer.LookUpGridResult;
  const GridDto = useSelector(Grid);
  const GridAll = (state: RootState) =>
    state.practiceGridReducer.LookUpGridResultAll;
  ...
  const refresh = () => {
    closeModal();
    GetPracticeGrid(query);
  };

  //PAGINAZIONE E RISULTATI FILTRAGGIO
  const { query, setQuery, next, back } = useResourceTableCrud(
    paginationQueryTipologiche,
    isPermesso ? GetPracticeGrid : undefined
  );
  ...
  const {
    ...
    GetPracticeCreateResource,
    GetPracticeEditResource,
    deletePractice,
    refresh
  );
  ...
  // useEffect(() => {
  //   GetPracticeGrid(paginationQueryTipologiche);
  //   GetPracticeGridALL();
  // }, []);
  ...
  //     const result = await GetRelatedRecordsPractice(id);
  //     if (result.data != null) {
    ...
    >
      ...
      >
        <DialogTitle className="d-flex justify-content-center">
          <div className="col-12">
            <h4 className="mb-0">{edit ? " Edit Practice" : "Add Practice"}</h4>
      ...
      <div className="headerPage row mx-0 my-3 justify-content-between">
        <div className="d-flex flex-row align-items-center">
          <h3 className="voda-bold text-dark">Practice</h3>
        </div>
        <div className="">
          <button
