...
import {
  ...
  GetFeedbackLogsGrid,
} from "../Redux/Action/FeedbackLogs/FeedbackLogsCommonAction";
import {
  FeedbackLogsDtoGrid,
  FeedbackLogsQueryObjectGrid,
} from "../Model/FeedbackLogs";
import FeedbackLogsGrid from "../screen/FeedbackLogs/FeedbackLogsGrid";
import { setNotification } from "../Redux/Action/NotificationAction";
import { NotifyType } from "../Redux/Reducer/NotificationReducer";

export let paginationQuery: FeedbackLogsQueryObjectGrid = {
  sortBy: "",
...
const FeedbackLogsContainer: React.FC = (props) => {
  //STATE CONFIRM
  ...
  const Grid = (state: RootState) =>
    state.FeedbackLogsGridReducer.FeedbackLogsGridResult;
  ...
  const refresh = () => {
    ...
    closeModal();
    GetFeedbackLogsGrid(query).then(() =>
  ...
  useEffect(() => {
    ...
    if (location.state != null && location.state !== undefined) {
      ...
      if (localState.id != null) {
        ...
        let copy = { ...query } as FeedbackLogsQueryObjectGrid;
        copy.principalId = localState.id;
        setQuery(copy);
        GetFeedbackLogsGrid(copy).then((x) => {
          setLoader("REMOVE", "GetFeedbackLogsGrid");
        });
      }
  ...
  const closeModalSetup = (changed: boolean) => {
    if (isVisibleModalPreview) {
      ...
      GetFeedbackLogsGrid(query).then((x) => setIsVisibleModalSetup(false));
    }
  };

  const DownloadReport = async () => {
    let result = await DownloadFeedbackLogs(query);
    if (result !== undefined) {
    ...
    <div className="pageContainer">
      ...
      <div className="headerPage row mx-0 justify-content-between">
        <div className="d-flex flex-row align-items-center">
          <h3 className="voda-bold">Feedback Logs</h3>
        </div>
...
export default FeedbackLogsContainer;
