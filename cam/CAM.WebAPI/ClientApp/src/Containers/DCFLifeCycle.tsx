...
import {
  DCFLifeCycleDtoCreate,
  DCFLifeCycleDtoGrid,
  DCFLifeCycleDtoUpdate,
...
import { GetDCFLifeCycleGrid } from "../Redux/Action/DCFLifeCycle/DCFLifeCycleGridAction";
import { RootState } from "../Redux/Store/rootStore";
import DCFLifeCycleGrid from "../screen/DCFLifeCycle/DCFLifeCycleGrid";
...
import { GetDCFLifeCycleReport } from "../Redux/Action/DCFLifeCycle/DCFLifeCycleDownloadAction";
import { GoArrowLeft } from "react-icons/go";
import { useTheme } from "../Context/ThemeContext";

export let paginationQuery: DCFLifeCycleQueryObjectGrid = {
  identityid: [],
  opCo: [],
  oem: [],
  networkFunction: [],
...
const DCFLifeCycle: React.FC = (props) => {
  //STATE CONFIRM
  ...
  const [activeTab, setActiveTab] = useState<string>("LCM");

  //DTO
  const [data, setData] = useState<DCFLifeCycleDtoGrid[] | undefined>([]);
  const Grid = (state: RootState) =>
    state.dcfLifeCycleGridReducer.DCFLifeCycleGridResult;
  ...
  const { query, setQuery, next, back } = useResourceTableCrud(
    paginationQuery,
    isPermesso ? GetDCFLifeCycleGrid : undefined
  );
  ...
  const refresh = () => {
    ...
    closeModal();
    GetDCFLifeCycleGrid(query).then(() =>
  ...
  const {
    ...
    setLocalState,
    Restore,
  } = useOperationTableCrud<DCFLifeCycleDtoUpdate, DCFLifeCycleDtoCreate>(
  ...
  useEffect(() => {
    let newQuery = { ...paginationQuery };
    newQuery.categoryType = [];
    switch (activeTab) {
      case "Assets":
        newQuery.categoryType = ["2"];
        break;
      case "LCM":
        newQuery.categoryType = ["3"];
        break;
      case "Component":
        newQuery.categoryType = ["5"];
        break;
      case "Identity":
        newQuery.categoryType = ["4"];
        break;
      default:
        newQuery.categoryType = [];
    }

    setLoader("Fillter", "GetDCFLifeCycleGrid");
    setQuery(newQuery);
  }, [activeTab]);
  ...
  useEffect(() => {
    ...
    if (location.state != null && location.state !== undefined) {
      ...
      if (localState?.id != null) {
        ...
        let copy = { ...query } as DCFLifeCycleQueryObjectGrid;
        copy.identityid = [];
        copy.identityid?.push(localState?.id);
        copy.principalId = localState?.id;
        setQuery(copy);
        GetDCFLifeCycleGrid(copy).then((x) =>
          setLoader("REMOVE", "GetDCFLifeCycleGrid")
        );
      }
      ...
      // GetDCFLifeCycleGrid(paginationQuery).then((x) =>
      //   setLoader("REMOVE", "GetDCFLifeCycleGrid")
      // );
    }
    setLoader("REMOVE", "GetDCFLifeCycleGrid");
  ...
  const closeModalSetup = (changed: boolean) => {
    GetDCFLifeCycleGrid(query).then((x) => setIsVisibleModalSetup(false));
  };

  const InvocheDownload = async () => {
    let result = await GetDCFLifeCycleReport(query);
    if (result !== undefined) {
    ...
    <div className="pageContainer">
      ...
      <div className="headerPage row mx-0 justify-content-between">
        <div className="d-flex flex-row align-items-center">
          ...
          <h3 className="voda-bold">DCF Life Cycle</h3>
...
"
      ...
      <Tabs
        id="dcf-life-cycle-tabs"
        activeKey={activeTab}
        onSelect={(k) => setActiveTab(k || "LCM")}
        className="mb-3"
      >
        <Tab eventKey="LCM" title="LCM">
          {activeTab === "LCM" && (
            <>
              <DCFLifeCycleGrid
                data={data}
