...
import { useManageNetworkPlans } from "./MenuItem";
...
const ManageNetworkPlan = () => {
  ...
  useEffect(() => {
    setMenuDescriptions(getDescriptionForOption("Manage Network Plan"));
  }, []);
  ...
  useEffect(() => {
    if (userPrefrenceDetails) {
      const getMenusList = getMenusForOption("Manage Network Plan");
  ...
  const getDescriptionForOption = (option: string) => {
    return option === "Manage Network Plan"
      ? { title: "Manage Network Plan", description: " ", sub: "" }
      : {
...
export default ManageNetworkPlan;
