...
import {
  DownloadAuditTrails,
  GetAuditTrailsGrid,
} from "../Redux/Action/AuditTrails/AuditTrailsCommonAction";
import {
  AuditTrailsDtoGrid,
  AuditTrailsQueryObjectGrid,
} from "../Model/AuditTrails";
import AuditTrailsGrid from "../screen/AuditTrails/AuditTrailsGrid";
import { setNotification } from "../Redux/Action/NotificationAction";
import { NotifyType } from "../Redux/Reducer/NotificationReducer";

export let paginationQuery: AuditTrailsQueryObjectGrid = {
  sortBy: "",
...
const AuditTrailsContainer: React.FC = (props) => {
  //STATE CONFIRM
  ...
  //DTO
  const [data, setData] = useState<AuditTrailsDtoGrid[] | undefined>([]);
  const Grid = (state: RootState) =>
    state.AuditTrailsGridReducer.AuditTrailsGridResult;
  ...
  useEffect(() => {
    ...
    if (location.state != null && location.state !== undefined) {
      ...
      if (localState.id != null) {
        ...
        let copy = { ...query } as AuditTrailsQueryObjectGrid;
        copy.principalId = localState.id;
        setQuery(copy);
        GetAuditTrailsGrid(copy).then((x) => {
          setLoader("REMOVE", "GetAuditTrailsGrid");
        });
      }
  ...
  const closeModalSetup = (changed: boolean) => {
    if (isVisibleModalPreview) {
      ...
      GetAuditTrailsGrid(query).then((x) => setIsVisibleModalSetup(false));
    }
  };

  const DownloadReport = async () => {
    let result = await DownloadAuditTrails(query);
    if (result !== undefined) {
    ...
    <div className="pageContainer">
      ...
      <div className="headerPage row mx-0 justify-content-between">
        <div className="d-flex flex-row align-items-center">
          <h3 className="voda-bold">Audit Trails</h3>
        </div>
