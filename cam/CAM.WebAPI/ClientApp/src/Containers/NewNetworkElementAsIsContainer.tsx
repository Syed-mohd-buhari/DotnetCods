...
import { GetNewNetworkElementAsIsReport } from "../Redux/Action/NetworkElementAsIs/NetworkElementAsIsDownloadAction";
import { GetNetworkElementAsIsEditResource } from "../Redux/Action/NetworkElementAsIs/NetworkElementAsIsEditAction";
import { GetNewNetworkElementAsIsGrid } from "../Redux/Action/NetworkElementAsIs/NetworkElementAsIsGridAction";
import { RootState } from "../Redux/Store/rootStore";
import NewNetworkElementAsIsGrid from "../screen/NetworkElementAsIs/NewNetworkElementAsIsGrid";
...
const NewNetworkElementAsIs: React.FC = (props) => {
  //STATE CONFIRM
  ...
  const Grid = (state: RootState) =>
    state.newnetworkElementAsIsGridReducer.NewNetworkElementAsIsGridResult;
  ...
  const { query, setQuery, next, back } = useResourceTableCrud(
    paginationQuery,
    isPermesso ? GetNewNetworkElementAsIsGrid : undefined
  );
  ...
  const refresh = () => {
    setLoader("ADD", "GetNewNetworkElementAsIsGrid");
    closeModal();
    GetNewNetworkElementAsIsGrid(query).then(() =>
      setLoader("REMOVE", "GetNewNetworkElementAsIsGrid")
    );
  };
  ...
  useEffect(() => {
    ...
    if (location.state != null && location.state !== undefined) {
      ...
      if (localState?.id != null) {
        ...
        setQuery(copy);
        GetNewNetworkElementAsIsGrid(copy).then((x) =>
          setLoader("REMOVE", "GetNewNetworkElementAsIsGrid")
        );
      }
      ...
      // GetNewNetworkElementAsIsGrid(paginationQuery).then((x) =>
      //   setLoader("REMOVE", "GetNewNetworkElementAsIsGrid")
      // );
    }
    setLoader("REMOVE", "GetNewNetworkElementAsIsGrid");
  ...
  const closeModalSetup = (changed: boolean) => {
    GetNewNetworkElementAsIsGrid(query).then((x) =>
      setIsVisibleModalSetup(false)
    );
  };

  const InvocheDownload = async () => {
    let result = await GetNewNetworkElementAsIsReport(query);
    if (result !== undefined) {
    ...
    <div className="pageContainer">
      ...
      <div className="headerPage row mx-0 justify-content-between">
        <div className="d-flex flex-row align-items-center">
          ...
          <h3 className="voda-bold">Network Element</h3>
        ...
        <div className="d-flex">
          {/* {!readonly && (
            ...
            >
              ...
              <span className="fz-14">New Network Element</span>
            </button>
          )} */}
...
export default NewNetworkElementAsIs;
