...
import {
  ResourceKeyMasterDtoCreate,
  ResourceKeyMasterDtoGrid,
  ResourceKeyMasterDtoUpdate,
...
import { GetResourceKeyMasterGrid } from "../Redux/Action/ResourceKeyMaster/ResourceKeyMasterGridAction";
import { RootState } from "../Redux/Store/rootStore";
import ResourceKeyMasterGrid from "../screen/ResourceKeyMaster/ResourceKeyMasterGrid";
...
import { GetResourceKeyMasterReport } from "../Redux/Action/ResourceKeyMaster/ResourceKeyMasterDownloadAction";
import { GoArrowLeft } from "react-icons/go";
import { useTheme } from "../Context/ThemeContext";

export let paginationQuery: ResourceKeyMasterQueryObjectGrid = {
  identityid: [],
  opCo: [],
  oem: [],
  networkFunction: [],
...
const ResourceKeyMaster: React.FC = (props) => {
  //STATE CONFIRM
  ...
  const Grid = (state: RootState) =>
    state.resourceKeyMasterGridReducer.ResourceKeyMasterGridResult;
  ...
  const { query, setQuery, next, back } = useResourceTableCrud(
    paginationQuery,
    isPermesso ? GetResourceKeyMasterGrid : undefined
  );
  ...
  const refresh = () => {
    ...
    closeModal();
    GetResourceKeyMasterGrid(query).then(() =>
  ...
  useEffect(() => {
    ...
    if (location.state != null && location.state !== undefined) {
      ...
      if (localState?.id != null) {
        ...
        let copy = { ...query } as ResourceKeyMasterQueryObjectGrid;
        copy.identityid = [];
        copy.identityid?.push(localState?.id);
        copy.principalId = localState?.id;
        setQuery(copy);
        GetResourceKeyMasterGrid(copy).then((x) =>
          setLoader("REMOVE", "GetResourceKeyMasterGrid")
        );
      }
      ...
      // GetResourceKeyMasterGrid(paginationQuery).then((x) =>
      //   setLoader("REMOVE", "GetResourceKeyMasterGrid")
      // );
    }
    setLoader("REMOVE", "GetResourceKeyMasterGrid");
  ...
  const closeModalSetup = (changed: boolean) => {
    GetResourceKeyMasterGrid(query).then((x) => setIsVisibleModalSetup(false));
  };

  const InvocheDownload = async () => {
    let result = await GetResourceKeyMasterReport(query);
    if (result !== undefined) {
    ...
    <div className="pageContainer">
      ...
      <div className="headerPage row mx-0 justify-content-between">
        <div className="d-flex flex-row align-items-center">
          ...
          <h3 className="voda-bold">Resource Key Master Details</h3>
...
export default ResourceKeyMaster;
