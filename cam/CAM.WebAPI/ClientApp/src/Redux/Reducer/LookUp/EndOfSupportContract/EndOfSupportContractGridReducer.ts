...
export const EndOfSupportContractGridReducer = (state = initState, action: { type: string, payload: LookUpGrid }) => {
    switch (action.type) {
        case "GET_GRID_END_OF_SUPPORT_CONTRACT":
            {
                return { ...state, LookUpGridResult: action.payload.LookUpGridResult }
            }
        case "GET_GRID_END_OF_SUPPORT_CONTRACT_ALL":
            {
                return { ...state, LookUpGridResultAll: action.payload.LookUpGridResult }
            }
        case "GET_FILTER_END_OF_SUPPORT_CONTRACT":
