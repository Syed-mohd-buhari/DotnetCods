...
export const LCMPlannedActionResourceGridReducer = (state = initState, action: { type: string, payload: LookUpGrid }) => {
    switch (action.type) {
        case "GET_GRID_LCM_PLANNED_ACTION_RESOURCE":
            {
                return { ...state, LookUpGridResult: action.payload.LookUpGridResult }
            }
        case "GET_GRID_LCM_PLANNED_ACTION_RESOURCE_ALL":
            {
                return { ...state, LookUpGridResultAll: action.payload.LookUpGridResult }
            }
        case "GET_FILTER_LCM_PLANNED_ACTION_RESOURCE":
