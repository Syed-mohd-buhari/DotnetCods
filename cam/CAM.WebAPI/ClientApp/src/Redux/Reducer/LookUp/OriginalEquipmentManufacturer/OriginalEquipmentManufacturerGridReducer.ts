...
export const OriginalEquipmentManufacturerGridReducer = (state = initState, action: { type: string, payload: LookUpGrid }) => {
    switch (action.type) {
        case "GET_GRID_ORIGINAL_EQUIPMENT_MANUFACTURER":
            {
                return { ...state, LookUpGridResult: action.payload.LookUpGridResult }
            }
        case "GET_GRID_ORIGINAL_EQUIPMENT_MANUFACTURER_ALL":
            {
                return { ...state, LookUpGridResultAll: action.payload.LookUpGridResult }
            }
        case "GET_FILTER_ORIGINAL_EQUIPMENT_MANUFACTURER":
