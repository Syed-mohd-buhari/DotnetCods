...
export async function GetNonTemsFNTReportExport(
  queryFilter?: FNTReportQueryObjectGrid
) {
  setLoader("ADD", "GetNonTemsFNTReportExport");
  ...
  try {
    res = await ApiCallWithErrorHandling<Promise<ReturnFile>>(() =>
      api.NonTemsFNTReportExportReport(queryFilter ?? {}, "excel")
    );
    ...
    setLoader("REMOVE", "GetNonTemsFNTReportExport");
    return result;
  } catch (error) {
    rootStore.dispatch(
      setNotification({
