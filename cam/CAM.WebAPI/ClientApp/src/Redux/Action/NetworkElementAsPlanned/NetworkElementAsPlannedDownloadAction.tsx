...
export async function GetNetworkElementAsPlannedReport(queryFilter?: NetworkElementAsPlannedQueryObjectGrid) {
	setLoader("ADD", "GetNetworkElementAsPlannedReport");
	let api = new NetworkElementAsPlannedApi();
	let res: ReturnFile | undefined;
	try {
			res = await ApiCallWithErrorHandling<Promise<ReturnFile>>(() =>
				api.networkElementAsPlannedExportReport(
					queryFilter ?? {}
				)
			);
		...
		setLoader("REMOVE", "GetNetworkElementAsPlannedReport");
