...
import { GetFilterColumnNFVISwCompatible } from "../../Redux/Action/NFVISoftwareCompatible/NFVISoftwareCompatibleGridAction";
...
interface Props {
  ...
  data: NFVISwCompatibleDtoGrid[] | undefined;
  pagination: NFVISwCompatibleQueryObjectGrid | undefined;
  renderGrid: RenderDetail[];
  orphanColor?: boolean;
}

let firstIndex, secondIndex, thirdIndex;
const NFVISoftwareCompatibleGrid: React.FC<Props> = (props) => {
  ...
  const getFiltersData = (state: RootState) =>
    state.NFVISwCompatibleGridReducer.filter;
  ...
  const {
    ...
    isFiltriAttivati,
  } = useFilterTableCrud<NFVISwCompatibleQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumnNFVISwCompatible,
    props.pagination
  );
            ...
            >
              ...
              >
                <>
                  ...
                  <Dropdown.Item
                    onClick={() => props.action.Delete(dropdownStates["rowId"])}
                  >
                    Delete
                  </Dropdown.Item>
                </>
          ...
          <table className="table-responsive" tabIndex={-1}>
            ...
            <tbody>
              {data?.map((item, index) => (
                <tr
                  className={`dati `}
                  key={index + "-" + item.nfviSoftwareCompatibilityId}
                  onDoubleClick={(e) =>
                    toggleDropdown(item.nfviSoftwareCompatibilityId, item, e)
                  }
                >
                  {props.renderGrid
                    .filter((x) => x.show)
                    ...
                    {!readonly && (
                      <div className="d-inline mx-2 cursor-pointer">
                        <img
                          ...
                          onClick={(e) => {
                            toggleDropdown(
                              item.nfviSoftwareCompatibilityId,
                              item,
                              e
                            );
                          }}
...
export default NFVISoftwareCompatibleGrid;
