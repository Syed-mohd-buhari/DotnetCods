...
import { GetFilterColumMajorHardwareBuild } from "../../Redux/Action/MajorHardwareBuild/MajorHardwareBuildGridAction";
...
interface Props {
  ...
  data: MajorHardwareBuildDtoGrid[] | undefined;
  pagination: MajorHardwareBuildQueryObjectGrid | undefined;
...
const MajorHardwareGrid: React.FC<Props> = (props) => {
  ...
  const {
    ...
    isFiltriAttivati,
  } = useFilterTableCrud<MajorHardwareBuildQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumMajorHardwareBuild,
    props.pagination
  );
          ...
          <tbody>
            {data?.map((item, index) => (
              <tr
                ...
                key={item.majorHardwareBuildId}
                onDoubleClick={(e) =>
                  toggleDropdown(
                    item.majorHardwareBuildId,
                    item.deleted,
                    item.orphan,
                    e
                  )
                }
                id={index === 0 ? "tourGrid_doubleClickHwRow" : "tourGrid"}
              >
                {props.renderGrid
                  .sort((a, b) => a.order - b.order)
                  .filter((x) => x.show)
...
export default React.memo(MajorHardwareGrid);
