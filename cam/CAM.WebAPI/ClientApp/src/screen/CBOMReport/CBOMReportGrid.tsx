...
import { CBOMReportDtoGrid } from "../../Model/CBOMReport";
...
interface Props {
  ...
  data: CBOMReportDtoGrid[] | undefined;
...
const CBOMReportGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<CBOMReportDtoGrid[] | undefined>([]);
  ...
  const getFiltersData = (state: RootState) =>
    state.CBOMReportGridReducer.filter;
        ...
        >
          ...
          >
            ...
            <tbody>
              {data?.map((item, index) => {
                return (
                  <React.Fragment key={index + "-" + item.cnfName}>
                    <tr className={`dati`} key={index + "-" + item.cnfName}>
                      {props?.renderGrid
                        .filter((x) => x.show)
                        .sort((a, b) => a.order - b.order)
                        .map((td, i) =>
