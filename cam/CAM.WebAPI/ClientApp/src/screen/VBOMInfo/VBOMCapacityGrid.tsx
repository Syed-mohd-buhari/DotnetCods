...
const VBOMInfoGrid: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    if (props.instanceId !== null) {
      setData(
        props?.data
          ?.filter((res) => res.vnfInfoId === props.infoId)?.[0]
          ?.["_instanceDtoGrid"]?.filter(
            (res) => res.vnfVmInstanceId === props.instanceId
          )?.[0]?.["_capacityDtoGrid"]
      );
    } else
      setData(
        props?.data?.[0]?.["_instanceDtoGrid"]?.[0]?.["_capacityDtoGrid"]
      );
    calculateBodyWidths(thRefs, firstIndex, secondIndex, thirdIndex);
      ...
      <div className="listaApparatiContainer mx-0 col-12 p-0 justify-content-center">
        <div className="mx-0 px-0 flex-row" style={{ position: "relative" }}>
          <table className="table-responsive overRightHeight" tabIndex={-1}>
            ...
            <tbody>
              {data?.map((item, index) => {
                const isExpanded = expandedRows.includes(item.vnfVmCapacityId);
