...
interface Props {
  ...
  data: VBOMClusterInfoDtoGrid[] | undefined;
...
const VBOMClusterInfoGrid: React.FC<Props> = (props) => {
  ...
  const getFiltersData = (state: RootState) =>
    state.VBOMClusterInfoGridReducer.filter;
      ...
      <div className="listaApparatiContainer mx-0 col-12 p-0 justify-content-center">
        <div className="mx-0 px-0 flex-row" style={{ position: "relative" }}>
          ...
          <table className="table-responsive" tabIndex={-1}>
            ...
            <tbody>
              {data?.map((item, index) => {
                const isExpanded = expandedRows.includes(item.vnfClusterInfoId);
                const visibleColumns = infoFilteredList
                  .filter((x) => x.show)
                  .sort((a, b) => a.order - b.order);
                const rowClass = index % 2 !== 0 ? "row-even" : "row-odd";
                const activeRow =
                  item?.vnfClusterInfoId === props.infoId ? true : false;
                return (
                  <React.Fragment key={index + "-" + item.vnfClusterInfoId}>
                    <tr
                      className={`dati ${rowClass}  ${
                        activeRow ? "activeStateRow" : ""
                      }`}
                      key={index + "-" + item.vnfClusterInfoId}
                      onClick={(e) =>
                        props.action.onInfoIdChange(item.vnfClusterInfoId)
                      }
                      onDoubleClick={(e) =>
                        toggleDropdown(item.vnfClusterInfoId, item, e)
                      }
                    >
                      {infoFilteredList
                        .filter((x) => x.show)
