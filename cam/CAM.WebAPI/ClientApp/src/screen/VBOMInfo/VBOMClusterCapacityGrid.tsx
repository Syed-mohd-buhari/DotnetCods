...
import { VBOMInfoDtoGrid, VbomCapacityQueryDto } from "../../Model/VBOMInfo";
import {
...
interface Props {
  action: {
    ...
    onCapacityIdChange(id: number | undefined): any;
...
const VBOMClusterCapacityGrid: React.FC<Props> = (props) => {
  ...
  const getFiltersData = (state: RootState) =>
    state.VBOMClusterCapacityGridReducer.filter;
  ...
  useEffect(() => {
    if (props.instanceId !== null) {
      setData(
        props?.data?.filter((res) => res.vnfInfoId === props.instanceId)?.[0]?.[
          "_vnfVbomCapacityDtoGrid"
        ]
      );
    } else setData(props?.data?.[0]?.["_vnfVbomCapacityDtoGrid"]);
    calculateBodyWidths(thRefs, firstIndex, secondIndex, thirdIndex);
  }, [props.data, props.instanceId]);
  ...
  useEffect(() => {
    ...
    const capacityFilteredList = props.renderGrid.map((item) => {
      if (capacityRenderList.includes(item.propertyName)) {
        return { ...item, show: true };
      } else {
        return { ...item, show: false };
      }
    });
    setRenderGridState(capacityFilteredList);
  }, [props.renderGrid]);
    ...
    <>
      ...
      <div className="listaApparatiContainer mx-0 col-12 p-0 justify-content-center">
        <div className="mx-0 px-0 flex-row" style={{ position: "relative" }}>
            ...
            <Dropdown
              ...
              style={
                dropdownStates["rowId"]
                  ? {
                      position: "absolute",
                      top: `${dropdownPosition.top - 572}px`,
                      right: `260px`,
          ...
          <table
            className="table-responsive overRightHeightCapacityTable"
            tabIndex={-1}
          >
            <thead>
            ...
            <tbody>
              {data?.map((item, index) => {
                ...
                const activeRow =
                  item?.vnfVmCapacityId === props.capacityId ? true : false;
                return (
                  <React.Fragment key={index + "-" + item.vnfVmCapacityId}>
                    <tr
                      className={`dati ${rowClass} ${
                        activeRow ? "activeStateRow" : ""
                      }`}
                      key={index + "-" + item.vnfVmCapacityId}
                      onClick={(e) =>
                        props.action.onCapacityIdChange(item.vnfVmCapacityId)
                      }
                      onDoubleClick={(e) =>
                        toggleDropdown(props.infoId, item, e)
                      }
                    >
                      {renderGridState
                        ?.filter((x) => x.show)
...
export default VBOMClusterCapacityGrid;
