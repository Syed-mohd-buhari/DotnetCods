...
const VBOMInfoGrid: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    if (props.infoId !== null) {
      setData(
        props?.data?.filter((res) => res.vnfInfoId === props.infoId)?.[0]?.[
          "_instanceDtoGrid"
        ]
      );
    } else setData(props?.data?.[0]?.["_instanceDtoGrid"]);
    calculateBodyWidths(thRefs, firstIndex, secondIndex, thirdIndex);
      ...
      <div className="listaApparatiContainer mx-0 col-12 p-0 justify-content-center">
        <div className="mx-0 px-0 flex-row" style={{ position: "relative" }}>
          ...
          >
            ...
            <tbody>
              {data?.map((item, index) => {
                const isExpanded = expandedRows.includes(item.vnfVmInstanceId);
                const visibleColumns = renderGridState
                  .filter((x) => x.show)
                  .sort((a, b) => a.order - b.order);
                const rowClass = index % 2 !== 0 ? "row-even" : "row-odd";
                const activeRow =
                  item?.vnfVmInstanceId === props.instanceId ? true : false;
                return (
                  <React.Fragment key={index + "-" + item.vnfVmInstanceId}>
                    <tr
                      className={`dati ${rowClass}  ${
                        activeRow ? "activeStateRow" : ""
                      }`}
                      key={index + "-" + item.vnfVmInstanceId}
                      onClick={(e) =>
                        props.action.onInstanceIdChange(item.vnfVmInstanceId)
                      }
                    >
                      {renderGridState
                        ?.filter((x) => x.show)
