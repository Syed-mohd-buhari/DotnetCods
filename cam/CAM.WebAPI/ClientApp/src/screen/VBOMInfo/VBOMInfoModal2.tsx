...
const OrganizationInfoModal: React.FC<Props> = (props) => {
  ...
  return (
    <div className="col-12">
      ...
      <div>
        <CustomAccordion title="Info" disableToggle={true}>
          <CustomMUITable columns={columns1} data={rows} size="small" />
        </CustomAccordion>

        <CustomAccordion title="Interface">
                    ...
                    <TableBody>
                      {row?.history?.map((h) => (
                        <TableRow key={h.date}>
                          ...
                          <TableCell>{h.customerId}</TableCell>
