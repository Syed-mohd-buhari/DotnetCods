...
const NetworkElementAsPlannedActivity: React.FC<Props> = (props) => {
  ...
  const onChangePlannedActivityResourceId = async (
    ...
    const ruleActivityDetailsNetworkElement =
      target?.ruleActicvityDetailsEditAsset as number;
    const exportable = target.exportable as boolean;

    //SE PROVIENI DA N.E. AS PLANNED
    if (props.isFromNetworkElement !== false) {
      switch (ruleActivityDetailsNetworkElement) {
        ...
        case 6:
          ...
          copy.activityDetails = target.activityDetailsEditAsset;
          copy.activityDetailsText = target.activityDetailsEditAsset;
    ...
    if (
      copy.activityDetails !== undefined &&
      stringIsNullOrEmpty(copy.activityDetails)
    ) {
      setActivityDetailsManual(true);
      // copy.activityDetails = "";
      // copy.activityDetailsText = "";
  ...
  const getPlannedPAId = (resObj) => {
    ...
    if (
      ...
      if (
        ...
        copy.activityDetails =
          hwSw[0]?.value.activityDetailsAddAsset ?? copy.activityDetails;
        copy.activityDetailsText =
          hwSw[0]?.value.activityDetailsAddAsset ?? copy.activityDetailsText;
      } else if (
        ...
        copy.activityDetails =
          sw[0]?.value.activityDetailsAddAsset ?? copy.activityDetails;
        copy.activityDetailsText =
          sw[0]?.value.activityDetailsAddAsset ?? copy.activityDetailsText;
      }
    }
    return copy;
  };
...
export default NetworkElementAsPlannedActivity;
