...
import { GetFilterColumAssetAsisSd } from "../../Redux/Action/Report/AssetAsisSdGridAction";
...
interface Props {
  ...
  data: AssetAsisSdDtoGrid[] | undefined;
  pagination: AssetAsisSdQueryObjectGrid | undefined;
  renderGrid: RenderDetail[];
}

let firstIndex, secondIndex, thirdIndex;
const AssetAsisSdGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<AssetAsisSdDtoGrid[] | undefined>([]);
  const { readonly, isPermesso } = useAuth();
  const getFiltersData = (state: RootState) =>
    state.assetAsisSdGridReducer.filter;
  ...
  } = useFilterTableCrud<AssetAsisSdQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumAssetAsisSd,
    props.pagination
  );
...
export default AssetAsisSdGrid;
