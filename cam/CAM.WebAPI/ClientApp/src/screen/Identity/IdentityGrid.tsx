...
import { IdentityDtoGrid, IdentityQueryObjectGrid } from "../../Model/Identity";
import { GetFilterColumIdentity } from "../../Redux/Action/Identity/IdentityGridAction";
...
interface Props {
  ...
  data: IdentityDtoGrid[] | undefined;
  pagination: IdentityQueryObjectGrid | undefined;
  renderGrid: RenderDetail[];
  orphanColor?: boolean;
}

let firstIndex, secondIndex, thirdIndex;
const IdentityGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<IdentityDtoGrid[] | undefined>([]);
  const filterData = useSelector(
    (state: RootState) => state.identityGridReducer.filter
  );
  const {
    filtriAttivi,
    resetFilter,
    ...
    isFiltriAttivati,
  } = useFilterTableCrud<IdentityQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumIdentity,
    props.pagination
  );
  ...
  useEffect(() => {
    isFiltriAttivati && props.action.setIsFiltriAttivati(isFiltriAttivati);
  }, [isFiltriAttivati]);
