...
import {
  CBOMClusterInfoDtoGrid,
  CBOMClusterInfoQueryObjectGrid,
  CBOMDtoGrid,
} from "../../Model/CBOM";
import { GetFilterColumnCBOMClusterInfo } from "../../Redux/Action/CBOM/CBOMGridAction";
...
interface Props {
  ...
  data: CBOMClusterInfoDtoGrid[] | undefined;
  pagination: CBOMClusterInfoQueryObjectGrid | undefined;
...
const CBOMClusterInfoGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<CBOMClusterInfoDtoGrid[] | undefined>([]);
  ...
  const getFiltersData = (state: RootState) =>
    state.CBOMClusterInfoGridReducer.filter;
  ...
  const {
    ...
    isFiltriAttivati,
  } = useFilterTableCrud<CBOMClusterInfoQueryObjectGrid>(
    props.action.Filter,
  ...
  // );
  const infoFilteredList = props?.renderGrid;
  return (
    <>
            ...
            <Dropdown
              ...
              style={
                dropdownStates["rowId"]
                  ? {
                      position: "absolute",
                      top: `${dropdownPosition.top - 290}px`,
          ...
          <table className="table-responsive" tabIndex={-1}>
            ...
            <tbody>
              {data?.map((item, index) => {
                ...
                const activeRow =
                  item?.cnfClusterInfoId === props.infoId ? true : false;
                return (
                  <React.Fragment key={index + "-" + item.cnfClusterInfoId}>
                    <tr
                      className={`dati ${rowClass}  ${
                        activeRow ? "activeStateRow" : ""
                      }`}
                      key={index + "-" + item.cnfClusterInfoId}
                      onClick={(e) =>
                        props.action.onInfoIdChange(item.cnfClusterInfoId)
                      }
                      onDoubleClick={(e) =>
                        toggleDropdown(item.cnfClusterInfoId, item, e)
                      }
                    >
                      {infoFilteredList
                        .filter((x) => x.show)
...
export default CBOMClusterInfoGrid;
