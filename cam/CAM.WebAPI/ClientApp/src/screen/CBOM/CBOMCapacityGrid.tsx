...
const CBOMGrid: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    if (props.instanceId !== null) {
      setData(
        props?.data
          ?.filter((res) => res.cnfInfoId === props.infoId)?.[0]
          ?.["_cbomClusterInfoDtoGrid"]?.filter(
            (res) => res.cnfClusterInfoInstanceId === props.instanceId
          )?.[0]?.["_cbomClusterCapacityDtoGrid"]
      );
    } else
      setData(
        props?.data?.[0]?.["_cbomClusterInfoDtoGrid"]?.[0]?.[
    ...
    <>
      ...
      <div className="listaApparatiContainer mx-0 col-12 p-0 justify-content-center">
        <div className="mx-0 px-0 flex-row" style={{ position: "relative" }}>
          <table className="table-responsive overRightHeight" tabIndex={-1}>
            <thead>
            ...
            <tbody>
              {data?.map((item, index) => {
                const isExpanded = expandedRows.includes(item.cnfCapacityId);
