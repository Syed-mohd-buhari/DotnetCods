...
import { GetFilterColumnCBOMCnfCapacityFilter } from "../../Redux/Action/CBOM/CBOMGridAction";
...
const CBOMGrid: React.FC<Props> = (props) => {
  ...
  const getFiltersData = (state: RootState) =>
    state.CBOMClusterCapacityGridReducer.filter;
  ...
  const {
    ...
    props.action.Filter,
    GetFilterColumnCBOMCnfCapacityFilter,
    props.pagination
  );
  ...
  useEffect(() => {
    if (props.instanceId !== null) {
      setData(
        props?.data?.filter(
          (res) => res.cnfPodInfoId === props.instanceId
        )?.[0]?.["_cnfCapacityDtoGrid"]
      );
    } else setData(props?.data?.[0]?.["_cnfCapacityDtoGrid"]);
    calculateBodyWidths(thRefs, firstIndex, secondIndex, thirdIndex);
          ...
          >
            ...
            <tbody>
              {data?.map((item, index) => {
                ...
                const activeRow =
                  item?.cnfCapacityId === props.capacityId ? true : false;
                return (
                  <React.Fragment key={index + "-" + item.cnfCapacityId}>
                    <tr
                      className={`dati ${rowClass} ${
                        activeRow ? "activeStateRow" : ""
                      }`}
                      key={index + "-" + item.cnfCapacityId}
                      onClick={(e) =>
                        props.action.onCapacityIdChange(item.cnfCapacityId)
                      }
                      onDoubleClick={(e) =>
