...
import { DaAssetMigrationDto } from "../../Model/LookUp/AssetMigrationModels";
...
const ServiceLevelPlannedActivity: React.FC<Props> = (props) => {
  ...
  const dtoNewResourceState = (state: RootState) => {
    ...
    return root?.plannedActivityCreateDto ?? root;
  };
  ...
  useEffect(() => {
    console.log("serviceDropdownOptions updated:", serviceDropdownOptions);
  }, [serviceDropdownOptions]);

  useEffect(() => {
    if (
      formData?.serviceMasterResource &&
      typeof formData.serviceMasterResource === "object"
    ) {
      const options = dictionaryToArray(formData.serviceMasterResource).map(
        (item) => ({
          key: item.key,
          value: item.value,
          label: item.value,
      ...
      setServiceDropdownOptions(options);
    }
  }, [formData?.serviceMasterResource]);
  ...
  const createForm = async () => {
    ...
    if (props.isReleaseDetailUnKnown) {
      ...
      const result: any = await GetServicePlanCreateResource();
                ...
                <div className="row col-12 px-0 mx-0 mt-3">
                  ...
                  <div className="col-6">
                    {/* <MultiSelectComponent
                      ...
                      disabled={edit}
                      value={(props.dcFsResource ?? [])
                        .filter(
                          (item) =>
                            Array.isArray(props.dcFsResource) &&
                            props.dcFsResource.includes(item.key)
                        )
                        .map((item) => ({
                          label: stripHtml(item.value),
                          value: item.key,
                        }))}
                      options={(props.dcFsResource ?? []).map((item) => ({
                        label: stripHtml(item.value),
                        value: item.key,
                      }))}
                    ...
                    <label className="labelForm voda-bold w-100">
                      ...
                      <Select
                        ...
                        options={
                          props.dcFsResource &&
                          dictionaryToArray(props.dcFsResource).sort((a, b) =>
                        ...
                        value={
                          props.dcFsResource
                            ? dictionaryToArray(props.dcFsResource).filter(
...
export default ServiceLevelPlannedActivity;
