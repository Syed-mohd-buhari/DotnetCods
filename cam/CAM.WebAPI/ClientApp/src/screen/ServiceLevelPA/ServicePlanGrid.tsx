...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<ServicePlanQueryObjectGrid> | undefined): any;
  };
...
const ServicePlanGrid: React.FC<Props> = (props) => {
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.servicePlanId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                      ...
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Edit(item.servicePlanId)
                              }
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.servicePlanId)
                              }
                            >
                              Delete
                            </Dropdown.Item>
