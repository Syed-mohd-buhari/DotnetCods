...
const DesignPlannedActivity: React.FC<Props> = (props) => {
  ...
  const [isRefactorDC, setisRefactorDC] = useState(false);
  ...
  const [refactorEnabled, setRefactorEnabled] = useState<boolean>(false);
  const [decommissionAllNodes, setDecommissionAllNodes] =
    useState<boolean>(false);
  const [isPlannedDcfAutoFilled, setIsPlannedDcfAutoFilled] =
    useState<boolean>(false);
  const thAction = {
        ...
        dictionaryToArrayPlannedActivityResourceDto(
          ...
          (x) =>
            x.value.plannedActivityResourceId ===
              formData?.plannedActivityResourceId && x.value.ruleLinkedDc === 38
        ).length === 1;
  ...
  const autoFillPlannedDcfFromCurrent = () => {
    let copy = { ...formData } as PlannedActivityDtoUpdate;
    copy.plannedDesignComponentFamilyId = props.designComponentFamilyId;
    setFormData(copy);
    setIsPlannedDcfAutoFilled(true);
    setDecommissionAllNodes(true);
        ...
        dictionaryToArrayPlannedActivityResourceDto(
          ...
          (res) =>
            res.value.plannedActivityResourceId ===
              x?.plannedActivityResourceId && res.value.ruleLinkedDc === 38
        ).length === 1;
      return {
      ...
      case 13:
        return (
          <AssetsDetailsContainer
            ...
            skipInitialApiCall={isRefactorDC}
            decommissionAll={decommissionAllNodes}
          />
        );
  ...
  useEffect(() => {
    setIsPlannedDcfAutoFilled(false);
  }, [formData?.plannedActivityResourceId]);
                  ...
                  >
                    ...
                    <td className="actions">
                      <div className="d-flex flex-row">
                        ...
                        {item.plannedActivityId !== undefined && !isPACheck ? (
                          <button
                            type="button"
                            title="Update Planned Activity Status"
                            className="btn btn-link"
                            onClick={() => {
                              if (item.plannedActivityId) {
                                openModalStatus(item.plannedActivityId);
                                setPlannedDcfId(
                                  item.plannedDesignComponentFamilyId ?? 0
                                );
                              }
                            }}
                          >
                    ...
                    <div className="row">
                          ...
                          <>
                            <div className="col-md-12">
                              ...
                              {plannedResourceSelected?.ruleLinkedDc === 19 &&
                                !edit && (
                                  <Tooltip
                                    title="Planned Design Component Family will be copied from Current Design Component Family.
                                     Kindly use Manage Assets to decommission the assets."
                                    arrow
                                  >
                                    <span>
                                      <button
                                        type="button"
                                        className="voda-bold btn btn-danger px-3 btnHeader mt-2 "
                                        style={{ marginLeft: "24px" }}
                                        onClick={() => {
                                          setConfirm({
                                            title: "Confirm Decommission",
                                            message:
                                              "Are you sure you want to decommission all nodes? Planned Design Component Family will be copied from Current Design Component Family,Kindly use Manage Assets to decommission the assets.",
                                            button: "OK",
                                            item: 0,
                                            isOpen: true,
                                            actions: {
                                              cancel: () =>
                                                setConfirm(stateConfirm),
                                              confirm: () => {
                                                autoFillPlannedDcfFromCurrent();
                                                setConfirm(stateConfirm);
                                              },
                                            },
                                          });
                                        }}
                                        disabled={isPlannedDcfAutoFilled}
                                      >
                        ...
                        <div className="col-md-12">
                          {plannedResourceSelected?.ruleLinkedDc === 17 ? (
                            <MultiSelectComponent
                              ...
                              disabled={edit}
                              value={(designComponentArray ?? [])
                                .filter(
                                  (item) =>
                                    Array.isArray(
                                      formData?.designComponentFamilyIdList
                                    ) &&
                                    formData?.designComponentFamilyIdList.includes(
                                      item.key
                                    )
                                )
                                .map((item) => ({
                              ...
                              options={(designComponentArray ?? []).map(
                                (item) => ({
                                  label: stripHtml(item.value),
                                  value: item.key,
                                })
                            ...
                            <label className="labelForm voda-bold w-100">
                              Planned Design Component Family
                              <span className="red">*</span>
                              <Select
                                menuPosition="fixed"
                                options={(
                                  plannnedesignComponentArray ?? []
                                ).map((item) => ({
                                  key: item.key,
                                  value: stripHtml(item.value),
                                }))}
                                value={
                                  (plannnedesignComponentArray ?? []).find(
                                    (x) =>
                                      x.key ===
                                      formData?.plannedDesignComponentFamilyId
                                  ) || null
                            ...
                            <>
                              <label className="labelForm voda-bold w-100">
                                ...
                                <Select
                                  menuPosition="fixed"
                                  options={
                                    designComponentSingleArray &&
                                    designComponentSingleArray?.filter(
                                      (x) =>
                                        x.key != props?.designComponentFamilyId
                                    )
                                  }
                                  value={
                                    formData?.designComponentFamilyResource
                                      ? formData?.designComponentFamilyResource?.find(
                                          (x) =>
                                            x.key ==
                                            formData?.plannedDesignComponentFamilyId
                                        )
                                  ...
                                  isClearable
                                  isDisabled={
                                    (edit && !refactorEnabled) ||
                                    isPlannedDcfAutoFilled
                                  }
                                  getOptionLabel={(option) => option.value}
                                  getOptionValue={(option) =>
                                    option.key.toString()
                                ...
                                <button
                                  ...
                                  onClick={async () => {
                                    setConfirm({
                                      ...
                                      message:
                                        "Are you sure you want to refactor? Please select the required Design for the Assets to be migrated to Target Platform.",
                                      button: "OK",
                                      item: 0,
                                      isOpen: true,
                                      actions: {
                                        ...
                                        confirm: async () => {
                                          ...
                                          if (formData?.plannedActivityId) {
                                            await GetAssetsPlatformMigrationGrid(
                                              {
                                            ...
                                            const gridData = (state as any)
                                              .daassetsMigrationGridReducer
                                              ?.DAAssetsMigrationGridResult;
                                  ...
                                  <tr className="intestazione">
                                    <th>
                                      <div className="h-100 d-flex align-items-center divFilter">
                                        <label>
                                          Selected Planned Design Component
                                          Family
                                        </label>
                                      </div>
                                    </th>
                      ...
                      {plannedResourceSelected?.ruleLinkedDc === 17 && (
                        <>
                          ...
                          <div className="col-6">
                            <label className="labelForm voda-bold w-100">
                              ...
                              validation.property?.includes(
                                ...
                                <label className="validation">
                                  *Project Description must have a value
                                </label>
                              ) : null}
                            </label>
                          </div>
                          ...
                          <div className="col-md-6">
                            <label className="labelForm voda-bold w-100">
                              ...
                              <input
                                type="text"
                                className="inputForm w-100"
                                placeholder="Delivery Project Name"
                                value={deliveryProjectNamePart}
                                onChange={(e) => {
                                  const newDeliveryName = e.currentTarget.value;
                                  const combined = `${newDeliveryName}-${wbsCodePart}`;
                                  setDeliveryProjectNamePart(newDeliveryName);
                                  onChange("deliveryProjectName", {
                                    currentTarget: {
                                      value: combined,
                                      type: "text",
                                    },
                          ...
                          <div className="col-md-6">
                            <label className="labelForm voda-bold w-100">
                              WBS CODE <span className="red">*</span>
                              <input
                                type="text"
                                className="inputForm w-100"
                                placeholder="WB CODE"
                                value={wbsCodePart}
                                onChange={(e) => {
                                  const newWbCode = e.currentTarget.value;
                                  const combined = `${deliveryProjectNamePart}-${newWbCode}`;
                                  setWbsCodePart(newWbCode);
                                  onChange("deliveryProjectName", {
                                    currentTarget: {
                                      value: combined,
                                      type: "text",
                              ...
                              {validation &&
                                validation.response === false &&
                                validation.property?.includes(
                                  "deliveryProjectName-WB"
                                ) && (
                                  <label className="validation">
                                    *WB CODE must have a value
                                  </label>
                                )}
                            </label>
                          </div>
                    ...
                    {isNoPA && (
                      <div className="row">
                        <div className="form-group col-6 pl-0">
                          <div className="col-12">
                            <label className="labelForm voda-bold w-100">
                              Reason for No Plan <span className="red">*</span>
                              <textarea
                                disabled={
                                  props.selectedDeploymentStatus
                                    ?.readOnlyPlannedActivity ||
...
export default DesignPlannedActivity;
