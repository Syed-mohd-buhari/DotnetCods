...
import { GetFilterColumHardwareConfiguration } from "../../Redux/Action/HardwareConfiguration/HardwareConfigurationGridAction";
...
interface Props {
  ...
  data: HardwareConfigurationDtoGrid[] | undefined;
  pagination: HardwareConfigurationQueryObjectGrid | undefined;
  renderGrid: RenderDetail[];
  orphanColor?: boolean;
}

let firstIndex, secondIndex, thirdIndex;
const HardwareConfigurationGrid: React.FC<Props> = (props) => {
  ...
  const getFiltersData = (state: RootState) =>
    state.hardwareConfigurationGridReducer.filter;
  ...
  const {
    ...
    isFiltriAttivati,
  } = useFilterTableCrud<HardwareConfigurationQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumHardwareConfiguration,
    props.pagination
  );
          ...
          <tbody>
            {data?.map((item, index) => (
              <tr
                ...
                key={item.hardwareconfigurationid}
              >
                {props.renderGrid
                  .filter((x) => x.show)
                  .sort((a, b) => a.order - b.order)
...
export default HardwareConfigurationGrid;
