...
import { GetFilterColumVNFClusterName } from "../../../Redux/Action/LookUp/VNFClusterName/VNFClusterNameGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<VNFClusterNameQueryObjectGrid> | undefined): any;
  };
  data: VNFClusterNameDtoGrid[] | undefined;
  pagination: VNFClusterNameQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const VNFClusterNameGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<VNFClusterNameDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.vNFClusterNameGridReducer.filter;
  ...
  } = useFilterTableCrud<VNFClusterNameQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumVNFClusterName,
    props.pagination
  );
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.clusterNameId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                      ...
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Edit(item.clusterNameId)
                              }
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.clusterNameId)
                              }
                            >
                              Delete
                            </Dropdown.Item>
...
export default VNFClusterNameGrid;
