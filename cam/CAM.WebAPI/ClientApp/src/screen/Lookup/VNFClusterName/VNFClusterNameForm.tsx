...
import {
  ...
  CreatVNFClusterName,
} from "../../../Redux/Action/LookUp/VNFClusterName/VNFClusterNameCreateAction";
import { deleteLocation } from "../../../Redux/Action/LookUp/Location/LocationDeleteAction";
import {
  GetVNFClusterNameEditResource,
  EditVNFClusterName,
} from "../../../Redux/Action/LookUp/VNFClusterName/VNFClusterNameEditAction";
import {
  GetInterVMTypeGrid,
...
import { VNFClusterNameDto } from "../../../Model/LookUp/VNFClusterName";
...
const VNFClusterNameForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<VNFClusterNameDto>(
    CreatVNFClusterName,
    EditVNFClusterName
  );

  const dtoEditResourceState = (state: RootState) =>
    state.vNFClusterNameEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.vNFClusterNameCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: VNFClusterNameDto) => {
    ...
    if (
      copy?.clusterDescription === null ||
      copy?.clusterDescription === undefined ||
      copy?.clusterDescription.trim() === ""
    ) {
      addInvalidProperty("clusterDescription");
    }
  ...
  const onChangeDefault = (checked: boolean) => {
    const copy = { ...formData } as VNFClusterNameDto;
  ...
  const OnChangeMultiSelect = (property: string, e: any) => {
    ...
    let copy = { ...formData } as VNFClusterNameDto;
  ...
  return (
    <div className="col-12">
      <Dialog
        ...
        aria-describedby="alert-dialog-clusterDescription"
        maxWidth="md"
        scroll="body"
        fullWidth={true}
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                Cluster Name <span className="red">*</span>
                <input
                  type="text"
                  onChange={(e) => onChange("clusterDescription", e)}
                  onKeyUp={(e) => onChange("clusterDescription", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.clusterDescription}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("clusterDescription") ? (
...
export default VNFClusterNameForm;
