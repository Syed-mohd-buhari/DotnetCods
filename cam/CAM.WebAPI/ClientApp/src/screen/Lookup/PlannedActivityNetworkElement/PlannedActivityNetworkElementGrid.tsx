...
import { GetFilterColumPlannedActivityNetworkElement } from "../../../Redux/Action/LookUp/PlannedActivityNetworkElement/PlannedActivityNetworkElementGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(
      obj:
        | SetStateAction<PlannedActivityNetworkElementQueryObjectGrid>
        | undefined
    ): any;
  };
  data: PlannedActivityNetworkElementDtoGrid[] | undefined;
  pagination: PlannedActivityNetworkElementQueryObjectGrid | undefined;
...
const PlannedActivityNetworkElementGrid: React.FC<Props> = (props) => {
  ...
  const getFiltersData = (state: RootState) =>
    state.plannedActivityNetworkElementGridReducer.filter;
  ...
  } = useFilterTableCrud<PlannedActivityNetworkElementQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumPlannedActivityNetworkElement,
    props.pagination
  );
      ...
      <div className="col-12 mx-0 px-0 py-3">
        <table className="w-100 table-responsive">
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.plannedActivityNetworkElementId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                      ...
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <Dropdown.Item
                            onClick={() =>
                              props.action.Edit(
                                item.plannedActivityNetworkElementId
                              )
                            }
                          >
                            Edit
...
export default PlannedActivityNetworkElementGrid;
