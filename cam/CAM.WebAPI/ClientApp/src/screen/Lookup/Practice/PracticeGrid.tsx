...
import { GetFilterColumPractice } from "../../../Redux/Action/LookUp/Practice/PracticeGridAction";
import {
...
const PracticeGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<TipologicaGridDto[] | undefined>([]);
  const getFiltersData = (state: RootState) => state.practiceGridReducer.filter;
              ...
              {props.renderGrid
                ...
                .map((item, i) =>
                  SelectFilterType(
                    ...
                    undefined,
                    undefined,
                    item.propertyName.toLowerCase() === "practicedescription"
                      ? "Description"
                      : item.propertyName === "practiceId"
                      ? "Id"
                      : undefined
                  )
                )}
                  ...
                  <td className="actions">
                    <Dropdown className="d-inline mx-2">
                      ...
                      <Dropdown.Menu>
                        <>
                          <Dropdown.Item
                            onClick={() => props.action.Edit(item.practiceId)}
                          >
                            Edit
                          </Dropdown.Item>
                          <Dropdown.Item
                            onClick={() => props.action.Delete(item.practiceId)}
                          >
                            Delete
                          </Dropdown.Item>
                        </>
