...
import { GetFilterColumVNFName } from "../../../Redux/Action/LookUp/VNFName/VNFNameGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<VNFNameQueryObjectGrid> | undefined): any;
  };
  data: VNFNameDtoGrid[] | undefined;
  pagination: VNFNameQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const VNFNameGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<VNFNameDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) => state.vNFNameGridReducer.filter;
  ...
  } = useFilterTableCrud<VNFNameQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumVNFName,
    props.pagination
  );
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.vnfNameId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                    ...
                    <div className="d-flex flex-row justify-content-end">
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() => props.action.Edit(item.vnfNameId)}
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.vnfNameId)
                              }
                            >
                              Delete
                            </Dropdown.Item>
