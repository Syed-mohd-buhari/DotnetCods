...
import {
  GetVNFNameCreateResource,
  CreatVNFName,
} from "../../../Redux/Action/LookUp/VNFName/VNFNameCreateAction";
import { deleteLocation } from "../../../Redux/Action/LookUp/Location/LocationDeleteAction";
import {
  GetVNFNameEditResource,
  EditVNFName,
} from "../../../Redux/Action/LookUp/VNFName/VNFNameEditAction";
import {
  GetInterVMTypeGrid,
...
import { VNFNameDto } from "../../../Model/LookUp/VNFName";
...
const VNFNameForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<VNFNameDto>(CreatVNFName, EditVNFName);

  const dtoEditResourceState = (state: RootState) =>
    state.vNFNameEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.vNFNameCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: VNFNameDto) => {
    ...
    if (
      copy?.vnfDescription === null ||
      copy?.vnfDescription === undefined ||
      copy?.vnfDescription.trim() === ""
    ) {
      addInvalidProperty("vnfDescription");
    }
  ...
  const onChangeDefault = (checked: boolean) => {
    const copy = { ...formData } as VNFNameDto;
  ...
  const OnChangeMultiSelect = (property: string, e: any) => {
    ...
    let copy = { ...formData } as VNFNameDto;
  ...
  return (
    <div className="col-12">
      <Dialog
        ...
        aria-describedby="alert-dialog-vnfDescription"
        maxWidth="md"
        scroll="body"
        fullWidth={true}
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                ...
                <input
                  type="text"
                  onChange={(e) => onChange("vnfDescription", e)}
                  onKeyUp={(e) => onChange("vnfDescription", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.vnfDescription}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("vnfDescription") ? (
          ...
          </div>
          {
            <div className="col-6">
              <label className="labelForm voda-bold   mb-0 w-100">
                ...
                <div className="d-flex">
                  <Select
                    ...
                    menuPosition={"fixed"}
                    options={
                      formData?.productResource &&
                      dictionaryToArray(formData.productResource)
                    }
                    value={
                      formData?.productResource &&
                      dictionaryToArray(formData.productResource).find(
                        (x) => x.key == formData.productId
                      )
                    }
                    onChange={(e) => onChangeSelect("productId", e)}
                    // onKeyUp={(e) => onChangeSelect("productId", e)}
              ...
              validation.property?.includes("productId") ? (
                <label className="validation">*Product cannot be empty!</label>
              ) : null}
            </div>
          }
          {/* <div className="col-6">
