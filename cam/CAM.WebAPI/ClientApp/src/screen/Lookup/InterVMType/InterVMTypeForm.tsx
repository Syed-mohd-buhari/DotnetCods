...
import {
  ...
  CreatInterVMType,
} from "../../../Redux/Action/LookUp/InterVMType/InterVMTypeCreateAction";
import { deleteLocation } from "../../../Redux/Action/LookUp/Location/LocationDeleteAction";
import {
  GetInterVMTypeEditResource,
  EditInterVMType,
} from "../../../Redux/Action/LookUp/InterVMType/InterVMTypeEditAction";
import {
  GetInterVMTypeGrid,
...
import { InterVMTypeDto } from "../../../Model/LookUp/InterVMType";
...
const InterVMTypeForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<InterVMTypeDto>(CreatInterVMType, EditInterVMType);

  const dtoEditResourceState = (state: RootState) =>
    state.interVMTypeEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.interVMTypeCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: InterVMTypeDto) => {
    ...
    if (
      copy?.interDescription === null ||
      copy?.interDescription === undefined ||
      copy?.interDescription.trim() === ""
    ) {
      addInvalidProperty("interDescription");
    }
  ...
  const onChangeDefault = (checked: boolean) => {
    const copy = { ...formData } as InterVMTypeDto;
  ...
  const OnChangeMultiSelect = (property: string, e: any) => {
    ...
    let copy = { ...formData } as InterVMTypeDto;
  ...
  return (
    <div className="col-12">
      <Dialog
        ...
        aria-describedby="alert-dialog-interDescription"
        maxWidth="md"
        scroll="body"
        fullWidth={true}
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                InterVMType Description <span className="red">*</span>
                <input
                  type="text"
                  onChange={(e) => onChange("interDescription", e)}
                  onKeyUp={(e) => onChange("interDescription", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.interDescription}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("interDescription") ? (
...
export default InterVMTypeForm;
