...
import { GetFilterColumInterVMType } from "../../../Redux/Action/LookUp/InterVMType/InterVMTypeGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<InterVMTypeQueryObjectGrid> | undefined): any;
  };
  data: InterVMTypeDtoGrid[] | undefined;
  pagination: InterVMTypeQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const InterVMTypeGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<InterVMTypeDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.interVMTypeGridReducer.filter;
  ...
  } = useFilterTableCrud<InterVMTypeQueryObjectGrid>(
    props.action.Filter,
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.interVmTypeId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                      ...
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Edit(item.interVmTypeId)
                              }
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.interVmTypeId)
                              }
                            >
                              Delete
                            </Dropdown.Item>
...
export default InterVMTypeGrid;
