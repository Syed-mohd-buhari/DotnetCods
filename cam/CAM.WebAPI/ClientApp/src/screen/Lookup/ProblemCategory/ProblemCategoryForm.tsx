...
import { CreatProblemCategory } from "../../../Redux/Action/LookUp/ProblemCategory/ProblemCategoryCreateAction";
...
const ProblemCategoryForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    CreatProblemCategory,
    EditProblemCategory
  );

  const dtoEditResourceState = (state: RootState) =>
    state.problemCategoryEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.problemCategoryCreateReducer.LookUpDtoCreate;
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                Problem Category<span className="red">*</span>
                <input
                  type="text"
                  onChange={(e) => onChange("problemCategoryDescription", e)}
                  onKeyUp={(e) => onChange("problemCategoryDescription", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.problemCategoryDescription}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("problemCategoryDescription") ? (
                <label className="validation">
                  *Problem Category Description is required
                </label>
              ) : null}
            </div>
          </div>
...
export default ProblemCategoryForm;
