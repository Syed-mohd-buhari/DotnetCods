...
import { GetFilterColumCNFName } from "../../../Redux/Action/LookUp/CNFName/CNFNameGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<CNFNameQueryObjectGrid> | undefined): any;
  };
  data: CNFNameDtoGrid[] | undefined;
  pagination: CNFNameQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const CNFNameGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<CNFNameDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) => state.cNFNameGridReducer.filter;
  ...
  } = useFilterTableCrud<CNFNameQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumCNFName,
    props.pagination
  );
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.cnfNameId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                    ...
                    <div className="d-flex flex-row justify-content-end">
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() => props.action.Edit(item.cnfNameId)}
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.cnfNameId)
                              }
                            >
                              Delete
                            </Dropdown.Item>
