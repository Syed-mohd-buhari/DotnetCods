...
import {
  GetCNFNameCreateResource,
  CreatCNFName,
} from "../../../Redux/Action/LookUp/CNFName/CNFNameCreateAction";
import { deleteLocation } from "../../../Redux/Action/LookUp/Location/LocationDeleteAction";
import {
  GetCNFNameEditResource,
  EditCNFName,
} from "../../../Redux/Action/LookUp/CNFName/CNFNameEditAction";
import {
  GetInterVMTypeGrid,
...
import { CNFNameDto } from "../../../Model/LookUp/CNFName";
...
const CNFNameForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<CNFNameDto>(CreatCNFName, EditCNFName);

  const dtoEditResourceState = (state: RootState) =>
    state.cNFNameEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.cNFNameCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: CNFNameDto) => {
    ...
    if (
      copy?.cnfDescription === null ||
      copy?.cnfDescription === undefined ||
      copy?.cnfDescription.trim() === ""
    ) {
      addInvalidProperty("cnfDescription");
    }
  ...
  const onChangeDefault = (checked: boolean) => {
    const copy = { ...formData } as CNFNameDto;
  ...
  const OnChangeMultiSelect = (property: string, e: any) => {
    ...
    let copy = { ...formData } as CNFNameDto;
  ...
  return (
    <div className="col-12">
      <Dialog
        ...
        aria-describedby="alert-dialog-cnfDescription"
        maxWidth="md"
        scroll="body"
        fullWidth={true}
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                ...
                <input
                  type="text"
                  onChange={(e) => onChange("cnfDescription", e)}
                  onKeyUp={(e) => onChange("cnfDescription", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.cnfDescription}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("cnfDescription") ? (
