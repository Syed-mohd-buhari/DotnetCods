...
import { GetFilterColumResponsibilityPhase } from "../../../Redux/Action/LookUp/ResponsibilityPhase/ResponsibilityPhaseGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(
      obj: SetStateAction<TipologicheQueryObjectGridRule> | undefined
    ): any;
  };
  data: TipologicaGridDtoRule[] | undefined;
  pagination: TipologicheQueryObjectGridRule | undefined;
  rules: { key: number; value: string }[];
  renderGrid: RenderDetail[];
}

const ResponsibilityPhaseGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<TipologicaGridDtoRule[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.responsibilityPhaseGridReducer.filter;
  ...
  } = useFilterTableCrud<TipologicheQueryObjectGridRule>(
    props.action.Filter,
    GetFilterColumResponsibilityPhase,
    props.pagination
  );
...
export default ResponsibilityPhaseGrid;
