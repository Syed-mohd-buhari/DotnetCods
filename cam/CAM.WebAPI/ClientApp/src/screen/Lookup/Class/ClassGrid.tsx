...
import { GetFilterColumClass } from "../../../Redux/Action/LookUp/Class/ClassGridAction";
import {
  ClassDtoGrid,
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<ClassQueryObjectGrid> | undefined): any;
  };
  data: ClassDtoGrid[] | undefined;
  pagination: ClassQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const ClassGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<ClassDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) => state.ClassGridReducer.filter;
  ...
  } = useFilterTableCrud<ClassQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumClass,
    props.pagination
  );
