...
import { GetFilterColumType } from "../../../Redux/Action/LookUp/Type/TypeGridAction";
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<TypeQueryObjectGrid> | undefined): any;
  };
  data: TypeDtoGrid[] | undefined;
  pagination: TypeQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const TypeGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<TypeDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) => state.TypeGridReducer.filter;
  ...
  } = useFilterTableCrud<TypeQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumType,
    props.pagination
  );
