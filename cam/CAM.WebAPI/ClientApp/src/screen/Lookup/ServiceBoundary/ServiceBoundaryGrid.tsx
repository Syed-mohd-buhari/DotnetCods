...
import { GetFilterColumServiceBoundary } from "../../../Redux/Action/LookUp/ServiceBoundary/ServiceBoundaryGridAction";
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<ServiceBoundaryQueryDto> | undefined): any;
  };
  data: ServiceBoundaryGridDto[] | undefined;
  pagination: ServiceBoundaryQueryDto | undefined;

  renderGrid: RenderDetail[];
}

const ServiceBoundaryGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<ServiceBoundaryGridDto[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.serviceBoundaryGridReducer.filter;
  ...
  } = useFilterTableCrud<ServiceBoundaryQueryDto>(
    props.action.Filter,
    GetFilterColumServiceBoundary,
    props.pagination
  );
      ...
      <div className="col-12 py-3">
        <table className="w-100 table-responsive">
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.serviceBoundaryId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                  ...
                  <td className="actions">
                    <Dropdown className="d-inline mx-2">
                      ...
                      <Dropdown.Menu>
                        <>
                          <Dropdown.Item
                            onClick={() =>
                              props.action.Edit(item.serviceBoundaryId)
                            }
                          >
                            Edit
                          </Dropdown.Item>
                    ...
                    {/* <div className="d-flex flex-row justify-content-end">
                      ...
                      <button
                        ...
                        className="btn btn-link "
                        onClick={() =>
                          props.action.onDelete(item.serviceBoundaryId)
                        }
                      >
                        <img
                          className="btnEdit"
...
export default ServiceBoundaryGrid;
