...
import { GetFilterColumPlanningActivityStatus } from "../../../Redux/Action/LookUp/PlanningActivityStatus/PlanningActivityStatusGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(
      obj:
        | SetStateAction<TipologicheQueryObjectGridProjectStatusCombinationRule>
        | undefined
    ): any;
  };
  data: TipologicaGridDtoProjectStatusCombinationRule[] | undefined;
  pagination:
    | TipologicheQueryObjectGridProjectStatusCombinationRule
    | undefined;
...
const PlanningActivityStatusGrid: React.FC<Props> = (props) => {
  ...
  const {
    ...
    useFilterTableCrud<TipologicheQueryObjectGridProjectStatusCombinationRule>(
      props.action.Filter,
      GetFilterColumPlanningActivityStatus,
      props.pagination
    );
          ...
          <thead>
            <tr className="intestazione">
              {props.renderGrid
                ...
                .filter((x) => x.show)
                .map((item, i) =>
                  item.propertyName != "projectStatusCombinationRule"
                    ? SelectFilterType(
                        item.propertyName,
                        item.type,
                  ...
                  {props.renderGrid
                    ...
                    .map((td, i) =>
                      td.propertyName === "projectStatusCombinationRule"
                        ? null //SelectGridType(rtnRuleGrid(item[td.propertyName] ?? 0, props.rulesProjectStatus), td.propertyName, td.type)
                        : SelectGridType(
...
export default PlanningActivityStatusGrid;
