...
import { GetFilterColumNetworkFunction } from "../../../Redux/Action/LookUp/NetworkFunction/NetworkFunctionGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(
      obj: SetStateAction<NetworkFunctionQueryObjectGrid> | undefined
    ): any;
  };
  data: NetworkFunctionDtoGrid[] | undefined;
  pagination: NetworkFunctionQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const NetworkFunctionGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<NetworkFunctionDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.networkFunctionGridReducer.filter;
  ...
  } = useFilterTableCrud<NetworkFunctionQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumNetworkFunction,
    props.pagination
  );
...
export default NetworkFunctionGrid;
