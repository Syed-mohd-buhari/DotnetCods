...
import {
  GetVMTypeNameCreateResource,
  CreatVMTypeName,
} from "../../../Redux/Action/LookUp/VMTypeName/VMTypeNameCreateAction";
import { deleteLocation } from "../../../Redux/Action/LookUp/Location/LocationDeleteAction";
import {
  GetVMTypeNameEditResource,
  EditVMTypeName,
} from "../../../Redux/Action/LookUp/VMTypeName/VMTypeNameEditAction";
import {
  GetVMTypeNameGrid,
  GetFilterColumVMTypeName,
} from "../../../Redux/Action/LookUp/VMTypeName/VMTypeNameGridAction";
import {
...
import { VMTypeNameDto } from "../../../Model/LookUp/VMTypeName";
...
const VMTypeNameForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<VMTypeNameDto>(CreatVMTypeName, EditVMTypeName);
  const dtoEditResourceState = (state: RootState) =>
    state.vMTypeNameEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.vMTypeNameCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: VMTypeNameDto) => {
    ...
    if (
      copy?.vmTypeDescription === null ||
      copy?.vmTypeDescription === undefined ||
      copy?.vmTypeDescription.trim() === ""
    ) {
  ...
  const onChangeDefault = (checked: boolean) => {
    const copy = { ...formData } as VMTypeNameDto;
  ...
  const OnChangeMultiSelect = (property: string, e: any) => {
    ...
    let copy = { ...formData } as VMTypeNameDto;
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                ...
                <input
                  type="text"
                  onChange={(e) => onChange("vmTypeDescription", e)}
                  onKeyUp={(e) => onChange("vmTypeDescription", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.vmTypeDescription}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("vmTypeDescription") ? (
          ...
          </div>
          {
            <div className="col-6">
              <label className="labelForm voda-bold   mb-0 w-100">
                ...
                <div className="d-flex">
                  <Select
                    ...
                    menuPosition={"fixed"}
                    options={
                      formData?.vnfNamResource &&
                      dictionaryToArray(formData.vnfNamResource)
                    }
                    value={
                      formData?.vnfNamResource &&
                      dictionaryToArray(formData.vnfNamResource).find(
                        (x) => x.key == formData.vnfNameId
                      )
                    }
                    onChange={(e) => onChangeSelect("vnfNameId", e)}
                    // onKeyUp={(e) => onChangeSelect("vnfNameId", e)}
              ...
              validation.property?.includes("vnfNameId") ? (
                <label className="validation">*VNF Name cannot be empty!</label>
              ) : null}
            </div>
          }
          {/* {
