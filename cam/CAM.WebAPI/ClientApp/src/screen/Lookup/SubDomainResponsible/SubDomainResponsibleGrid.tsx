...
import { GetFilterColumSubDomainResponsible } from "../../../Redux/Action/LookUp/SubDomainResponsible/SubDomainResponsibleGridAction";
import {
...
const SubDomainResponsibleGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<TipologicaGridDto[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.subDomainResponsibleGridReducer.filter;
  ...
  const {
    ...
    props.action.Filter,
    GetFilterColumSubDomainResponsible,
    props.pagination
  );
...
export default SubDomainResponsibleGrid;
