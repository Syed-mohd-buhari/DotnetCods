...
import { GetFilterColumVMWorkloadType } from "../../../Redux/Action/LookUp/VMWorkloadType/VMWorkloadTypeGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<VMWorkloadTypeQueryObjectGrid> | undefined): any;
  };
  data: VMWorkloadTypeDtoGrid[] | undefined;
  pagination: VMWorkloadTypeQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const VMWorkloadTypeGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<VMWorkloadTypeDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.vMWorkloadTypeGridReducer.filter;
  ...
  } = useFilterTableCrud<VMWorkloadTypeQueryObjectGrid>(
    props.action.Filter,
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.vmWorkLoadTypeId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                      ...
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Edit(item.vmWorkLoadTypeId)
                              }
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.vmWorkLoadTypeId)
                              }
                            >
                              Delete
                            </Dropdown.Item>
...
export default VMWorkloadTypeGrid;
