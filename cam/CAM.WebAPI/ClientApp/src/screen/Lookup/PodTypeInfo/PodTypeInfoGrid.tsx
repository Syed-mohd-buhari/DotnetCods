...
import { GetFilterColumPodTypeInfo } from "../../../Redux/Action/LookUp/PodTypeInfo/PodTypeInfoGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<PodTypeInfoQueryObjectGrid> | undefined): any;
  };
  data: PodTypeInfoDtoGrid[] | undefined;
  pagination: PodTypeInfoQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const PodTypeInfoGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<PodTypeInfoDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.podTypeInfoGridReducer.filter;
  ...
  } = useFilterTableCrud<PodTypeInfoQueryObjectGrid>(
    props.action.Filter,
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.podTypeInfoId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                      ...
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Edit(item.podTypeInfoId)
                              }
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.podTypeInfoId)
                              }
                            >
                              Delete
                            </Dropdown.Item>
...
export default PodTypeInfoGrid;
