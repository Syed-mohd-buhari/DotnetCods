...
import {
  ...
  CreatPodTypeInfo,
} from "../../../Redux/Action/LookUp/PodTypeInfo/PodTypeInfoCreateAction";
import { deleteLocation } from "../../../Redux/Action/LookUp/Location/LocationDeleteAction";
import {
  GetPodTypeInfoEditResource,
  EditPodTypeInfo,
} from "../../../Redux/Action/LookUp/PodTypeInfo/PodTypeInfoEditAction";
import {
  GetPodTypeInfoGrid,
  GetFilterColumPodTypeInfo,
...
import { PodTypeInfoDto } from "../../../Model/LookUp/PodTypeInfo";
...
const PodTypeInfoForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<PodTypeInfoDto>(CreatPodTypeInfo, EditPodTypeInfo);

  const dtoEditResourceState = (state: RootState) =>
    state.podTypeInfoEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.podTypeInfoCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: PodTypeInfoDto) => {
    ...
    if (
      copy?.podTypeInfoName === null ||
      copy?.podTypeInfoName === undefined ||
      copy?.podTypeInfoName.trim() === ""
    ) {
      addInvalidProperty("podTypeInfoName");
    }
  ...
  const onChangeDefault = (checked: boolean) => {
    const copy = { ...formData } as PodTypeInfoDto;
  ...
  const OnChangeMultiSelect = (property: string, e: any) => {
    ...
    let copy = { ...formData } as PodTypeInfoDto;
  ...
  return (
    <div className="col-12">
      <Dialog
        ...
        aria-describedby="alert-dialog-podTypeInfoName"
        maxWidth="md"
        scroll="body"
        fullWidth={true}
      ...
      >
        <div className="row col-12 px-0">
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                Pod Type Description <span className="red">*</span>
                <input
                  type="text"
                  onChange={(e) => onChange("podTypeInfoName", e)}
                  onKeyUp={(e) => onChange("podTypeInfoName", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.podTypeInfoName}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("podTypeInfoName") ? (
          ...
          <div className="form-group">
            <label className="labelForm voda-bold   mb-0 w-100">
              Pod Role Description <span className="red">*</span>
              <input
                type="text"
                onChange={(e) => onChange("podRoleDescription", e)}
                onKeyUp={(e) => onChange("podRoleDescription", e)}
                className="inputForm w-100"
                defaultValue={formData?.podRoleDescription}
              />
            </label>
            {validation &&
            validation.response === false &&
            validation.property?.includes("podRoleDescription") ? (
              <label className="validation">*Description is required</label>
            ) : null}
          </div>
          {/* <div className="col-6">
...
export default PodTypeInfoForm;
