...
import {
  ...
  CreatAssetMapInfo,
} from "../../../Redux/Action/LookUp/AssetMapInfo/AssetMapInfoCreateAction";
import { deleteLocation } from "../../../Redux/Action/LookUp/Location/LocationDeleteAction";
import {
  GetAssetMapInfoEditResource,
  EditAssetMapInfo,
} from "../../../Redux/Action/LookUp/AssetMapInfo/AssetMapInfoEditAction";
import {
  GetIntraVMTypeGrid,
...
import { AssetMapInfoDto } from "../../../Model/LookUp/AssetMapInfo";
...
const AssetMapInfoForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<AssetMapInfoDto>(CreatAssetMapInfo, EditAssetMapInfo);

  const dtoEditResourceState = (state: RootState) =>
    state.assetMapInfoEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.assetMapInfoCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: AssetMapInfoDto) => {
    ...
    if (
      copy?.omcAssetName === null ||
      copy?.omcAssetName === undefined ||
      copy?.omcAssetName.trim() === ""
    ) {
      addInvalidProperty("omcAssetName");
    }

    if (
      copy?.temsAssetName === null ||
      copy?.temsAssetName === undefined ||
      copy?.temsAssetName.trim() === ""
    ) {
      addInvalidProperty("temsAssetName");
    }

    if (
      copy?.enmAssetName === null ||
      copy?.enmAssetName === undefined ||
      copy?.enmAssetName.trim() === ""
    ) {
      addInvalidProperty("enmAssetName");
    }
    ...
    if (
      copy?.dataSourceName === null ||
      copy?.dataSourceName === undefined ||
      copy?.dataSourceName.trim() === ""
    ) {
      addInvalidProperty("dataSourceName");
    }
  ...
  const onChangeDefault = (checked: boolean) => {
    const copy = { ...formData } as AssetMapInfoDto;
  ...
  const OnChangeMultiSelect = (property: string, e: any) => {
    ...
    let copy = { ...formData } as AssetMapInfoDto;
      ...
      >
        <div className="row col-12 px-0">
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                OMC ASSET NAME <span className="red">*</span>
                <input
                  type="text"
                  onChange={(e) => onChange("omcAssetName", e)}
                  onKeyUp={(e) => onChange("omcAssetName", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.omcAssetName}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("omcAssetName") ? (
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                TEMS ASSET NAME <span className="red">*</span>
                <input
                  type="text"
                  onChange={(e) => onChange("temsAssetName", e)}
                  onKeyUp={(e) => onChange("temsAssetName", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.temsAssetName}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("temsAssetName") ? (
                <label className="validation">*Description is required</label>
              ) : null}
            </div>
          </div>
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                ENM ASSET NAME <span className="red">*</span>
                <input
                  type="text"
                  onChange={(e) => onChange("enmAssetName", e)}
                  onKeyUp={(e) => onChange("enmAssetName", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.enmAssetName}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("enmAssetName") ? (
                <label className="validation">*Description is required</label>
              ) : null}
            </div>
          </div>
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                ...
                <input
                  type="text"
                  onChange={(e) => onChange("site", e)}
                  onKeyUp={(e) => onChange("site", e)}
                  className="inputForm w-100"
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                DATA SOURCE NAME <span className="red">*</span>
                <input
                  type="text"
                  onChange={(e) => onChange("dataSourceName", e)}
                  onKeyUp={(e) => onChange("dataSourceName", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.dataSourceName}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("dataSourceName") ? (
                <label className="validation">*Description is required</label>
              ) : null}
            </div>
          </div>
...
export default AssetMapInfoForm;
