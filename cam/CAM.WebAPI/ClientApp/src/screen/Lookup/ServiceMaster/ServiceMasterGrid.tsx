...
import { GetFilterColumServiceMaster } from "../../../Redux/Action/LookUp/ServiceMaster/ServiceMasterGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<ServiceMasterQueryObjectGrid> | undefined): any;
  };
  data: ServiceMasterDtoGrid[] | undefined;
  pagination: ServiceMasterQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const ServiceMasterGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<ServiceMasterDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.serviceMasterGridReducer.filter;
  ...
  } = useFilterTableCrud<ServiceMasterQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumServiceMaster,
    props.pagination
  );
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.serviceMasterIndex}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                      ...
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Edit(item.serviceMasterIndex)
                              }
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.serviceMasterIndex)
                              }
                            >
                              Delete
                            </Dropdown.Item>
...
export default ServiceMasterGrid;
