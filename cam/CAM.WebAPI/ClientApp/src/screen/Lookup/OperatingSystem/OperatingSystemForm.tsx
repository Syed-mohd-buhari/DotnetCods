...
import { CreatOperatingSystem } from "../../../Redux/Action/LookUp/OperatingSystem/OperatingSystemCreateAction";
import { EditOperatingSystem } from "../../../Redux/Action/LookUp/OperatingSystem/OperatingSystemEditAction";
...
const OperatingSystemForm: React.FC<Props> = (props) => {
  const {
    formData,
    setFormData,
    Save,
    ...
    CreatOperatingSystem,
    EditOperatingSystem
  );

  const dtoEditResourceState = (state: RootState) =>
    state.operatingSystemEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.operatingSystemCreateReducer.LookUpDtoCreate;
      ...
      >
        <div className="row col-12 px-0">
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                Operating System <span className="red fz-20">*</span>
                <input
                  type="text"
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                OS Version <span className="red fz-20">*</span>
                <input
                  type="text"
                  onChange={(e) => onChange("operatingSystemVersion", e)}
                  onKeyUp={(e) => onChange("operatingSystemVersion", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.operatingSystemVersion}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("operatingSystemVersion") ? (
                <label className="validation">*OS Version is required</label>
              ) : null}
            </div>
          </div>
          <div className="col-12">
