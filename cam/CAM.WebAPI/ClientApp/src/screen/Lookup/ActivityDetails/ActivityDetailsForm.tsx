...
import {
  ...
  CreatActivityDetails,
} from "../../../Redux/Action/LookUp/ActivityDetails/ActivityDetailsCreateAction";
import { deleteActivityDetails } from "../../../Redux/Action/LookUp/ActivityDetails/ActivityDetailsDeleteAction";
import {
  GetActivityDetailsEditResource,
  EditActivityDetails,
} from "../../../Redux/Action/LookUp/ActivityDetails/ActivityDetailsEditAction";
import {
  GetActivityDetailGrid,
  GetFilterColumActivityDetail,
} from "../../../Redux/Action/LookUp/ActivityDetails/ActivityDetailsGridAction";
import {
  TipologicheQueryObjectGrid,
  TipologicaGridDto,
  TipologicaQueryDtoForVirtualized,
...
const ActivityDetailsForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<TipologicaGridDtoForVirtualized>(
    CreatActivityDetails,
    EditActivityDetails
  );

  const dtoEditResourceState = (state: RootState) =>
    state.activityDetailsEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.activityDetailsCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: TipologicaGridDtoForVirtualized) => {
              ...
              <label className="labelForm voda-bold   mb-0 w-100 widthAuto">
                <div className="switchContainer">
                  ...
                  <label className="switch">
                    <input
                      type="checkbox"
                      checked={formData?.forVirtualized}
                      onChange={(e) => onChange("forVirtualized", e)}
                    />
