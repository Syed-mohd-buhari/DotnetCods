...
import { GetFilterColumIntraVMType } from "../../../Redux/Action/LookUp/IntraVMType/IntraVMTypeGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<IntraVMTypeQueryObjectGrid> | undefined): any;
  };
  data: IntraVMTypeDtoGrid[] | undefined;
  pagination: IntraVMTypeQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const IntraVMTypeGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<IntraVMTypeDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.intraVMTypeGridReducer.filter;
  ...
  } = useFilterTableCrud<IntraVMTypeQueryObjectGrid>(
    props.action.Filter,
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.intraVmTypeId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                      ...
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Edit(item.intraVmTypeId)
                              }
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.intraVmTypeId)
                              }
                            >
                              Delete
                            </Dropdown.Item>
...
export default IntraVMTypeGrid;
