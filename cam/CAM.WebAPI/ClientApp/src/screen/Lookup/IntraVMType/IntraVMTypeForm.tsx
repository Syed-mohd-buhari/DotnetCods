...
import {
  ...
  CreatIntraVMType,
} from "../../../Redux/Action/LookUp/IntraVMType/IntraVMTypeCreateAction";
import { deleteLocation } from "../../../Redux/Action/LookUp/Location/LocationDeleteAction";
import {
  GetIntraVMTypeEditResource,
  EditIntraVMType,
} from "../../../Redux/Action/LookUp/IntraVMType/IntraVMTypeEditAction";
import {
  GetIntraVMTypeGrid,
...
import { IntraVMTypeDto } from "../../../Model/LookUp/IntraVMType";
...
const IntraVMTypeForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<IntraVMTypeDto>(CreatIntraVMType, EditIntraVMType);

  const dtoEditResourceState = (state: RootState) =>
    state.intraVMTypeEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.intraVMTypeCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: IntraVMTypeDto) => {
    ...
    if (
      copy?.intraDescription === null ||
      copy?.intraDescription === undefined ||
      copy?.intraDescription.trim() === ""
    ) {
      addInvalidProperty("intraDescription");
    }
  ...
  const onChangeDefault = (checked: boolean) => {
    const copy = { ...formData } as IntraVMTypeDto;
  ...
  const OnChangeMultiSelect = (property: string, e: any) => {
    ...
    let copy = { ...formData } as IntraVMTypeDto;
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                IntraVMType Description <span className="red">*</span>
                <input
                  type="text"
                  onChange={(e) => onChange("intraDescription", e)}
                  onKeyUp={(e) => onChange("intraDescription", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.intraDescription}
                />
              </label>
              {validation &&
...
export default IntraVMTypeForm;
