...
import { GetFilterColumVerticalResponsible } from "../../../Redux/Action/LookUp/VerticalResponsible/VerticalResponsibleGridAction";
import {
...
const VerticalResponsibleGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<TipologicaGridDto[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.verticalResponsibleGridReducer.filter;
  ...
  const {
    ...
    props.action.Filter,
    GetFilterColumVerticalResponsible,
    props.pagination
  );
...
export default VerticalResponsibleGrid;
