...
import { GetFilterColumNFVIStatus } from "../../../Redux/Action/LookUp/NFVIStatus/NFVIStatusGridAction";
import {
  NFVIStatusDtoGrid,
  NFVIStatusQueryObjectGrid,
} from "../../../Model/LookUp/NFVIStatus";
import { colors } from "./NFVIStatusForm";
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<NFVIStatusQueryObjectGrid> | undefined): any;
  };
  data: NFVIStatusDtoGrid[] | undefined;
  pagination: NFVIStatusQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const NFVIStatusGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<NFVIStatusDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.nFVIStatusGridReducer.filter;
  ...
  } = useFilterTableCrud<NFVIStatusQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumNFVIStatus,
    props.pagination
  );
  ...
  const rtnColorFromList = (value: string | "") => {
    return colors.find((x) => x.key === value)?.value;
  };
              ...
              data?.map((item, i) => (
                <tr className="dati" key={item.id}>
                  {props.renderGrid
                    ...
                    .filter((x) => x.show)
                    .map((td, i) =>
                      td.propertyName === "color"
                        ? SelectGridType(
                            rtnColorFromList(item[td.propertyName] ?? ""),
                            td.propertyName,
                            td.type,
