...
import {
  ...
  CreatCNFPriority,
} from "../../../Redux/Action/LookUp/CNFPriority/CNFPriorityCreateAction";
import { deleteLocation } from "../../../Redux/Action/LookUp/Location/LocationDeleteAction";
import {
  GetCNFPriorityEditResource,
  EditCNFPriority,
} from "../../../Redux/Action/LookUp/CNFPriority/CNFPriorityEditAction";
import {
  GetInterVMTypeGrid,
...
import { CNFPriorityDto } from "../../../Model/LookUp/CNFPriority";
...
const CNFPriorityForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<CNFPriorityDto>(CreatCNFPriority, EditCNFPriority);

  const dtoEditResourceState = (state: RootState) =>
    state.cNFPriorityEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.cNFPriorityCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: CNFPriorityDto) => {
  ...
  const onChangeDefault = (checked: boolean) => {
    const copy = { ...formData } as CNFPriorityDto;
  ...
  const OnChangeMultiSelect = (property: string, e: any) => {
    ...
    let copy = { ...formData } as CNFPriorityDto;
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                Priority Description <span className="red">*</span>
                <input
                  type="text"
...
export default CNFPriorityForm;
