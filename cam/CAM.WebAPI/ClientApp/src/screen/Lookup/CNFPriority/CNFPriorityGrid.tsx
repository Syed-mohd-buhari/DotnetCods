...
import { GetFilterColumCNFPriority } from "../../../Redux/Action/LookUp/CNFPriority/CNFPriorityGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<CNFPriorityQueryObjectGrid> | undefined): any;
  };
  data: CNFPriorityDtoGrid[] | undefined;
  pagination: CNFPriorityQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const CNFPriorityGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<CNFPriorityDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.cNFPriorityGridReducer.filter;
  ...
  } = useFilterTableCrud<CNFPriorityQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumCNFPriority,
    props.pagination
  );
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.cnfPriorityId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                      ...
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Edit(item.cnfPriorityId)
                              }
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.cnfPriorityId)
                              }
                            >
                              Delete
                            </Dropdown.Item>
...
export default CNFPriorityGrid;
