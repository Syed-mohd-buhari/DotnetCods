...
import { CreatDriver } from "../../../Redux/Action/LookUp/Driver/DriverCreateAction";
import { EditDriver } from "../../../Redux/Action/LookUp/Driver/DriverEditAction";
...
const DriverForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<TipologicaGridDto>(CreatDriver, EditDriver);

  const dtoEditResourceState = (state: RootState) =>
    state.driverEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.driverCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: TipologicaGridDto) => {
    ...
    if (
      copy?.bptDriverDetails === null ||
      copy?.bptDriverDetails === undefined ||
      copy?.bptDriverDetails.trim() === ""
    ) {
      addInvalidProperty("bptDriverDetails");
    }
      ...
      >
        <div className="row col-12 px-0">
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                Driver<span className="red">*</span>
                <input
                  type="text"
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                BPT Driver<span className="red">*</span>
                <input
                  type="text"
                  onChange={(e) => onChange("bptDriverDetails", e)}
                  onKeyUp={(e) => onChange("bptDriverDetails", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.bptDriverDetails}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("bptDriverDetails") ? (
                <label className="validation">*BPT Driver is required</label>
              ) : null}
            </div>
          </div>
