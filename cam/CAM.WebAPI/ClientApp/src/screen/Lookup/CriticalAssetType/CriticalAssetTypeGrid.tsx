...
import { GetFilterColumCriticalAssetType } from "../../../Redux/Action/LookUp/CriticalAssetType/CriticalAssetTypeGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(
      obj: SetStateAction<CriticalAssetTypeQueryObjectGrid> | undefined
    ): any;
  };
  data: CriticalAssetTypeDtoGrid[] | undefined;
  pagination: CriticalAssetTypeQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const CriticalAssetTypeGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<CriticalAssetTypeDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.criticalAssetTypeGridReducer.filter;
  ...
  } = useFilterTableCrud<CriticalAssetTypeQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumCriticalAssetType,
    props.pagination
  );
  ...
  return (
    <div className="listaApparatiContainer row mx-0 col-12 p-0 d-flex justify-content-center">
      <div className="mx-0 px-0" style={{ position: "relative" }}>
...
export default CriticalAssetTypeGrid;
