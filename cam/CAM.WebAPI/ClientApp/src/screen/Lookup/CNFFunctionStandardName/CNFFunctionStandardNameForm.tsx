...
import {
  ...
  CreatCNFFunctionStandardName,
} from "../../../Redux/Action/LookUp/CNFFunctionStandardName/CNFFunctionStandardNameCreateAction";
import { deleteLocation } from "../../../Redux/Action/LookUp/Location/LocationDeleteAction";
import {
  GetCNFFunctionStandardNameEditResource,
  EditCNFFunctionStandardName,
} from "../../../Redux/Action/LookUp/CNFFunctionStandardName/CNFFunctionStandardNameEditAction";
import {
  GetInterVMTypeGrid,
...
import { CNFFunctionStandardNameDto } from "../../../Model/LookUp/CNFFunctionStandardName";
...
const CNFFunctionStandardNameForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<CNFFunctionStandardNameDto>(
    CreatCNFFunctionStandardName,
    EditCNFFunctionStandardName
  );

  const dtoEditResourceState = (state: RootState) =>
    state.cNFFunctionStandardNameEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.cNFFunctionStandardNameCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: CNFFunctionStandardNameDto) => {
    ...
    if (
      ...
      copy?.functionName === undefined ||
      copy?.functionName.trim() === ""
    ) {
      addInvalidProperty("functionName");
    }
  ...
  const onChangeDefault = (checked: boolean) => {
    const copy = { ...formData } as CNFFunctionStandardNameDto;
  ...
  const OnChangeMultiSelect = (property: string, e: any) => {
    ...
    let copy = { ...formData } as CNFFunctionStandardNameDto;
  ...
  return (
    <div className="col-12">
      <Dialog
        ...
        aria-describedby="alert-dialog-functionName"
        maxWidth="md"
        scroll="body"
        fullWidth={true}
          ...
          <div className="col-6">
            <div className="form-group">
              <label className="labelForm voda-bold   mb-0 w-100">
                Function Name <span className="red">*</span>
                <input
                  type="text"
                  onChange={(e) => onChange("functionName", e)}
                  onKeyUp={(e) => onChange("functionName", e)}
                  className="inputForm w-100"
                  defaultValue={formData?.functionName}
                />
              </label>
              {validation &&
              validation.response === false &&
              validation.property?.includes("functionName") ? (
...
export default CNFFunctionStandardNameForm;
