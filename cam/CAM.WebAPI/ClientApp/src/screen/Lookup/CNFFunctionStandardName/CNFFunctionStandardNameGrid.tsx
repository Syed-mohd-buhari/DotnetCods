...
import { GetFilterColumCNFFunctionStandardName } from "../../../Redux/Action/LookUp/CNFFunctionStandardName/CNFFunctionStandardNameGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(
      obj: SetStateAction<CNFFunctionStandardNameQueryObjectGrid> | undefined
    ): any;
  };
  data: CNFFunctionStandardNameDtoGrid[] | undefined;
  pagination: CNFFunctionStandardNameQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const CNFFunctionStandardNameGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<
    CNFFunctionStandardNameDtoGrid[] | undefined
  >([]);
  const getFiltersData = (state: RootState) =>
    state.cNFFunctionStandardNameGridReducer.filter;
  ...
  } = useFilterTableCrud<CNFFunctionStandardNameQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumCNFFunctionStandardName,
    props.pagination
  );
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.functionStandardNameId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                      ...
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Edit(item.functionStandardNameId)
                              }
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.functionStandardNameId)
                              }
                            >
                              Delete
                            </Dropdown.Item>
...
export default CNFFunctionStandardNameGrid;
