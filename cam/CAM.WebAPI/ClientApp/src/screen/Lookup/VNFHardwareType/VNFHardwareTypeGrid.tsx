...
import { GetFilterColumVNFHardwareType } from "../../../Redux/Action/LookUp/VNFHardwareType/VNFHardwareTypeGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(
      obj: SetStateAction<VNFHardwareTypeQueryObjectGrid> | undefined
    ): any;
  };
  data: VNFHardwareTypeDtoGrid[] | undefined;
  pagination: VNFHardwareTypeQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const VNFHardwareTypeGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<VNFHardwareTypeDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.vNFHardwareTypeGridReducer.filter;
  ...
  } = useFilterTableCrud<VNFHardwareTypeQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumVNFHardwareType,
    props.pagination
  );
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.vnfHardwareId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                      ...
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Edit(item.vnfHardwareId)
                              }
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.vnfHardwareId)
                              }
                            >
                              Delete
                            </Dropdown.Item>
...
export default VNFHardwareTypeGrid;
