...
import { GetFilterColumCNFHardwareType } from "../../../Redux/Action/LookUp/CNFHardwareType/CNFHardwareTypeGridAction";
import {
...
interface Props {
  action: {
    ...
    Filter(
      obj: SetStateAction<CNFHardwareTypeQueryObjectGrid> | undefined
    ): any;
  };
  data: CNFHardwareTypeDtoGrid[] | undefined;
  pagination: CNFHardwareTypeQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const CNFHardwareTypeGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<CNFHardwareTypeDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.cNFHardwareTypeGridReducer.filter;
  ...
  } = useFilterTableCrud<CNFHardwareTypeQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumCNFHardwareType,
    props.pagination
  );
      ...
      <div className="col-12 py-3">
        ...
        >
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr className="dati" key={item.cnfHardwareId}>
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
                    .filter((x) => x.show)
                    .map((td, i) =>
                      ...
                      <Dropdown className="d-inline mx-2">
                        ...
                        <Dropdown.Menu>
                          <>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Edit(item.cnfHardwareId)
                              }
                            >
                              Edit
                            </Dropdown.Item>
                            <Dropdown.Item
                              onClick={() =>
                                props.action.Delete(item.cnfHardwareId)
                              }
                            >
                              Delete
                            </Dropdown.Item>
...
export default CNFHardwareTypeGrid;
