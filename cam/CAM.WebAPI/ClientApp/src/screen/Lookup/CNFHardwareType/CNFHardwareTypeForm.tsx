...
import {
  ...
  CreatCNFHardwareType,
} from "../../../Redux/Action/LookUp/CNFHardwareType/CNFHardwareTypeCreateAction";
import { deleteLocation } from "../../../Redux/Action/LookUp/Location/LocationDeleteAction";
import {
  GetCNFHardwareTypeEditResource,
  EditCNFHardwareType,
} from "../../../Redux/Action/LookUp/CNFHardwareType/CNFHardwareTypeEditAction";
import {
  GetInterVMTypeGrid,
...
import { CNFHardwareTypeDto } from "../../../Model/LookUp/CNFHardwareType";
...
const CNFHardwareTypeForm: React.FC<Props> = (props) => {
  ...
  const {
    ...
    setInputValue,
    promiseSelect,
  } = useFormTableCrud<CNFHardwareTypeDto>(
    CreatCNFHardwareType,
    EditCNFHardwareType
  );

  const dtoEditResourceState = (state: RootState) =>
    state.cNFHardwareTypeEditReducer.LookUpDtoEdit;
  const dtoNewResourceState = (state: RootState) =>
    state.cNFHardwareTypeCreateReducer.LookUpDtoCreate;
  ...
  const validazioneClient = (copy: CNFHardwareTypeDto) => {
  ...
  const onChangeDefault = (checked: boolean) => {
    const copy = { ...formData } as CNFHardwareTypeDto;
  ...
  const OnChangeMultiSelect = (property: string, e: any) => {
    ...
    let copy = { ...formData } as CNFHardwareTypeDto;
...
export default CNFHardwareTypeForm;
