...
import { GetFilterColumAssetType } from "../../../Redux/Action/LookUp/AssetType/AssetTypeGridAction";
import {
  AssetTypeDtoGrid,
...
interface Props {
  action: {
    ...
    Filter(obj: SetStateAction<AssetTypeQueryObjectGrid> | undefined): any;
  };
  data: AssetTypeDtoGrid[] | undefined;
  pagination: AssetTypeQueryObjectGrid | undefined;

  renderGrid: RenderDetail[];
}

const AssetTypeGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<AssetTypeDtoGrid[] | undefined>([]);
  const getFiltersData = (state: RootState) =>
    state.assetTypeGridReducer.filter;
  ...
  } = useFilterTableCrud<AssetTypeQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumAssetType,
    props.pagination
  );
  ...
  return (
    <div className="listaApparatiContainer row mx-0 col-12 p-0 d-flex justify-content-center">
      <div className="col-12 mx-0 px-0 py-3" style={{ position: "relative" }}>
