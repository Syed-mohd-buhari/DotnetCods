...
interface Props {
  ...
  data: OrganizationInfoDtoGrid[] | undefined;
...
const OrganizationInfoGrid: React.FC<Props> = (props) => {
  ...
  const getFiltersData = (state: RootState) =>
    state.organizationInfoGridReducer.filter;
  ...
  const {
    ...
    props.action.Filter,
    GetFilterColumOrganizationInfo,
    props.pagination
  );
          ...
          <table className="table-responsive" tabIndex={-1}>
            ...
            <tbody>
              {data?.map((item, index) => (
                <tr
                  className={`dati `}
                  key={index + "-" + item.organisationId}
                  onDoubleClick={(e) =>
                    toggleDropdown(item.organisationId, item, e)
                  }
                >
                  {props.renderGrid
                    .filter((x) => x.show)
...
export default OrganizationInfoGrid;
