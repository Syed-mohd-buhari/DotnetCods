...
import { GetNonTemsFilterColumFNTReport } from "../../Redux/Action/FNTReport/NonTemsFNTReportGridAction";
...
interface Props {
  ...
  data: FNTReportDtoGrid[] | undefined;
  pagination: FNTReportQueryObjectGrid | undefined;
  renderGrid: RenderDetail[];
  orphanColor?: boolean;
}

let firstIndex, secondIndex, thirdIndex;
const NonTemsFNTReportGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<FNTReportDtoGrid[] | undefined>([]);
  ...
  const getFiltersData = (state: RootState) =>
    state.nonTemsFntReportGridReducer.filter;
  ...
  const {
    ...
    isFiltriAttivati,
  } = useFilterTableCrud<FNTReportQueryObjectGrid>(
    props.action.Filter,
    GetNonTemsFilterColumFNTReport,
    props.pagination
  );
            ...
            <tbody>
              {data?.map((item, index) => (
                <tr
                  ...
                  key={index + "-" + item.temsFntReportId}
                >
                  {props.renderGrid
                    .filter((x) => x.show)
                    .sort((a, b) => a.order - b.order)
...
export default NonTemsFNTReportGrid;
