...
import { TestInfoDtoGrid, TestInfoQueryObjectGrid } from "../../Model/TestInfo";
import { GetFilterColumTestInfo } from "../../Redux/Action/TestInfo/TestInfoGridAction";
...
interface Props {
  ...
  data: TestInfoDtoGrid[] | undefined;
  pagination: TestInfoQueryObjectGrid | undefined;
  renderGrid: RenderDetail[];
  orphanColor?: boolean;
}

let firstIndex, secondIndex, thirdIndex;
const TestInfoGrid: React.FC<Props> = (props) => {
  ...
  const getFiltersData = (state: RootState) => state.testInfoGridReducer.filter;
  ...
  const {
    ...
    isFiltriAttivati,
  } = useFilterTableCrud<TestInfoQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumTestInfo,
    props.pagination
  );
          ...
          <table className="table-responsive" tabIndex={-1}>
            ...
            <tbody>
              {data?.map((item, index) => (
                <tr
                  className={`dati `}
                  key={index + "-" + item.systemVerificationProblemId}
                  onDoubleClick={(e) =>
                    toggleDropdown(item.systemVerificationProblemId, item, e)
                  }
                >
                  {props.renderGrid
                    .filter((x) => x.show)
                    ...
                    {!readonly && (
                      <div className="d-inline mx-2 cursor-pointer">
                        <img
                          ...
                          onClick={(e) => {
                            toggleDropdown(
                              item.systemVerificationProblemId,
                              item,
                              e
                            );
                          }}
