...
import { GetFilterColumReportNetworkLevel2 } from "../../Redux/Action/Report/ReportLcmExportGridAction";
...
import { NetworkLevel2ReportDtoGrid } from "../../Model/Report/ReportLcmExportModel";
...
interface Props {
  ...
  data: NetworkLevel2ReportDtoGrid[] | undefined;
  pagination: ReportNetworkLevel2QueryGrid | undefined;
  renderGrid: RenderDetail[];
}

let firstIndex, secondIndex, thirdIndex;
const LcmExportNetworkLevel2: React.FC<Props> = (props) => {
  const [data, setData] = useState<NetworkLevel2ReportDtoGrid[] | undefined>(
    []
  );
  const { readonly, isPermesso } = useAuth();
  const getFiltersData = (state: RootState) =>
    state.reportNetworkLevel2GridReducer.filter;
  ...
  } = useFilterTableCrud<ReportNetworkLevel2QueryGrid>(
    props.action.Filter,
    GetFilterColumReportNetworkLevel2,
    props.pagination
  );
...
export default LcmExportNetworkLevel2;
