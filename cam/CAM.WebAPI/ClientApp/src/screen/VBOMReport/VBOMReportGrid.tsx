...
import { VBOMReportDtoGrid } from "../../Model/VBOMReport";
...
interface Props {
  ...
  data: VBOMReportDtoGrid[] | undefined;
...
const VBOMReportGrid: React.FC<Props> = (props) => {
  const [data, setData] = useState<VBOMReportDtoGrid[] | undefined>([]);
  ...
  const getFiltersData = (state: RootState) =>
    state.VBOMReportGridReducer.filter;
  ...
  const {
    ...
    isFiltriAttivati,
  } = useFilterTableCrud<any>(props.action.Filter, undefined, props.pagination);
  ...
  return (
    <>
      ...
      <div
        className="listaApparatiContainer mx-0 col-12 p-0 justify-content-center"
        style={{ height: "20.9rem" }}
      >
        <div
          className="mx-0 px-0 flex-row"
          style={{ position: "relative", height: "20.9rem" }}
        >
          <table
          ...
          >
            <thead>
              <tr
                ...
                style={{ backgroundColor: "#cccccc !important" }}
              >
                {props?.renderGrid
                  .filter((x) => x.show)
            ...
            <tbody>
              {data?.map((item, index) => {
                return (
                  <React.Fragment key={index + "-" + item.vnfName}>
                    <tr className={`dati`} key={index + "-" + item.vnfName}>
                      {props?.renderGrid
                        .filter((x) => x.show)
                        .sort((a, b) => a.order - b.order)
                        .map((td, i) =>
