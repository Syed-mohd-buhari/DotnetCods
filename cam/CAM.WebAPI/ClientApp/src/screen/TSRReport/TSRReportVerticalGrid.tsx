...
import { GetFilterColumProductImportance } from "../../Redux/Action/LookUp/ProductImportance/ProductImportanceGridAction";
import {
...
const TSRReportVerticalGrid: React.FC<Props> = (props) => {
  ...
  const {
    ...
    props.action.Filter,
    GetFilterColumProductImportance,
    props.pagination
  );
          ...
          <tbody>
            {data &&
              data?.map((item, i) => (
                <tr
                  className="dati"
                  key={item.appSettingsConfigurationId}
                  style={{ position: "relative" }}
                  onDoubleClick={(e) =>
                    toggleDropdown(item.appSettingsConfigurationId, e)
                  }
                >
                  {props.renderGrid
                    .sort((a, b) => a.order - b.order)
...
export default TSRReportVerticalGrid;
