...
import { GetFilterColumComponentSwBuild } from "../../Redux/Action/ComponentSwBuild/ComponentSwBuildGridAction";
...
interface Props {
  action: {
    ...
    GetComponentSwCloneData(value: number): any;
  };
  data: ComponentSwBuildDtoGrid[] | undefined;
  pagination: ComponentSwBuildQueryObjectGrid | undefined;
...
const ComponentSwGrid: React.FC<Props> = (props) => {
  ...
  const getFiltersData = (state: RootState) =>
    state.componentSwBuildGridReducer.filter;
  ...
  const {
    ...
    isFiltriAttivati,
  } = useFilterTableCrud<ComponentSwBuildQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumComponentSwBuild,
    props.pagination
  );
            ...
            >
              {dropdownStates["isDeleted"] ? (
                ...
                <>
                  <Dropdown.Item
                    onClick={() =>
                      props.action.GetComponentSwCloneData(
                        dropdownStates["rowId"] ?? 0
                      )
                    }
                  >
                    Upgrade Component SW Version
                  </Dropdown.Item>
                  <Dropdown.Item
