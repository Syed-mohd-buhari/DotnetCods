...
interface Props {
  ...
  data: SoftwareComponentDtoGrid[] | undefined;
  pagination: SoftwareComponentQueryObjectGrid | undefined;
  renderGrid: RenderDetail[];
  orphanColor?: boolean;
}

let firstIndex, secondIndex, thirdIndex;
const SoftwareComponentGrid: React.FC<Props> = (props) => {
  console.log("Tems SoftwareComponentGrid props", props);
  ...
  const getFiltersData = (state: RootState) =>
    state.softwareComponentGridReducer.filter;
  ...
  const {
    ...
    isFiltriAttivati,
  } = useFilterTableCrud<SoftwareComponentQueryObjectGrid>(
    props.action.Filter,
    GetFilterColumSoftwareComponent,
    props.pagination
  );
          ...
          <tbody>
            {data?.map((item, index) => (
              <tr
                ...
                key={item.softwarewarecomponentid}
              >
                {props.renderGrid
                  .filter((x) => x.show)
                  .sort((a, b) => a.order - b.order)
...
export default SoftwareComponentGrid;
