...
import { GetRelatedRecordsMajorSoftwareBuild } from "../../../Redux/Action/MajorSoftwareBuild/MajorSoftwareBuildDeleteAction";
import { RelatedRecordsResponse } from "./ProductSoftwareTypes";
...
export interface DeleteModalProps {
  open: boolean;
  row: MajorSoftwareBuildDtoGrid | null;
  onClose: () => void;
  onConfirmed: (row: MajorSoftwareBuildDtoGrid) => void;
}
...
const DeleteModal: React.FC<DeleteModalProps> = ({
  ...
  const checkRelations = async () => {
    try {
      const result: any = await GetRelatedRecordsMajorSoftwareBuild(
        ...
        <DeleteModalCard sx={{ maxWidth: "560px" }}>
          ...
          >
            <Typography sx={{ fontSize: "14px", color: T.black }}>
              The {relatedData.data?.entityName ?? "Major Software Build"}
            </Typography>
            <Typography sx={{ fontSize: "14px", color: T.black }}>
              <strong>{relatedData?.data?.recordName ?? ""}</strong> is related
              to:
            </Typography>
