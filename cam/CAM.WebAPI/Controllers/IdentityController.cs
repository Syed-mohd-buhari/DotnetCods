...
namespace CAM.WebAPI.Controllers
{
    ...
    public class IdentityController : CamControllerBase 
    {
        ...
        public IdentityController(ILoggerManager logger, IHttpContextAccessor contextAccessor, IExportService exportService, IdentityManager manager
        ...
        public QueryResultDto<IDentitiesDtoGrid> GetIdentity([FromBody] IdentityQueryDto identityQueryDto )
        {
            try
            {
                if ((identityQueryDto.Opco == null) || (identityQueryDto.Opco != null && identityQueryDto.Opco.Count == 0))
                    identityQueryDto.Opco = _opcoDescriptionList;
                return _manager.FindWithCondition(identityQueryDto);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex);
        ...
        [HttpPost("Filter")]
        public List<FilterValueDto> GetFilterResult([FromQuery] string propertyName, [FromQuery] string propertyFilter, [FromBody] IdentityQueryDto designComponentFilterDto)
        {
            try
            {
        ...
        public FileContentResult ExportReport([FromBody] IdentityQueryDto dto)
        {
            try
            {
                dto.Page = 0;
                ...
                var result = _exportService.GetExcelFrom(reportSheets,
                     "Identity_Raw_" + DateTime.Now.ToShortDateString() + ".xlsx", data.GridRender);
