...
{
    [Route("api/PassThroughLcmSoftware")]
    [ApiController]
    ...
    public class PassThroughLcmSoftwareController : CamControllerBase 
    {
        private readonly SwPassThroughLcmManager _manager;
        ...
        public PassThroughLcmSoftwareController(ILoggerManager logger, IHttpContextAccessor contextAccessor, IExportService exportService, SwPassThroughLcmManager manager, CommonManager commonManager,
        ...
        [HttpPost("GetPassThroughLcmSoftwareReport")]
        public QueryResultDto<PassThroughLcmSoftwareDtoGrid> GetPassThroughReport([FromBody] PassThroughLcmQueryDto passThroughLcmQueryDto)
        {
            try
            {
        ...
        public FileContentResult ExportReport([FromBody] PassThroughLcmQueryDto dto)
        {
            try
            {
                ...
                var result = _exportService.GetExcelForLcmPassthroughSoftware(reportSheets, "PassThrough_LCM_Software_" + domainName + "_" + DateTime.Now.ToShortDateString() + ".xlsx", data.GridRender);
        ...
        public async Task<ResultDto> ImportXls(IFormFile file, [FromQuery] long nonTemsVertical)
        {
            ...
            try
            {
                var excelConfigurationEntity = _commonManager.GetExcelConfiguration("PassThroughLcmSoftware").Result;
                using var stream = file.OpenReadStream();

                return await _importService.ImportDataBasedOnTemplateConfigurationForXL(file, ConstantValueFilter.passThroughLcmSoftware, excelConfigurationEntity, nonTemsVertical);
